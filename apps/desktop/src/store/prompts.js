...
function keyedPromptStore() {
    const $all = atom({});
    const idOf = (value) => value.requestId;
    return {
...
const $approvalInlineAnchors = atom({});
...
export const sessionApprovalRequest = (sessionId) => computed(approval.$all, all => all[keyFor(sessionId)] ?? null);
export const sessionSudoRequest = (sessionId) => computed(sudo.$all, all => all[keyFor(sessionId)] ?? null);
export const sessionSecretRequest = (sessionId) => computed(secret.$all, all => all[keyFor(sessionId)] ?? null);
export function registerApprovalInlineAnchor(sessionId) {
    const key = keyFor(sessionId);
    const bump = (delta) => {
...
export const sessionApprovalInlineVisible = (sessionId) => computed($approvalInlineAnchors, anchors => (anchors[keyFor(sessionId)] ?? 0) > 0);
...
export const $activeSessionAwaitingInput = computed([$clarifyRequest, $approvalRequest, $sudoRequest, $secretRequest], (clarify, approval, sudo, secret) => Boolean(clarify || approval || sudo || secret));
 ...
 *  active one). */
export function sessionAwaitingInput(sessionId) {
...
export function clearAllPrompts(sessionId) {
    if (sessionId === undefined) {
        approval.reset();
        sudo.reset();
        secret.reset();
