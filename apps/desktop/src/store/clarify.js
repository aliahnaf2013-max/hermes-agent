...
export const $clarifyRequests = atom({});
...
export const $clarifyRequest = computed([$clarifyRequests, $activeSessionId], (requests, activeId) => requests[keyFor(activeId)] ?? null);
...
export const sessionClarifyRequest = (sessionId) => computed($clarifyRequests, requests => requests[keyFor(sessionId)] ?? null);
export function setClarifyRequest(request) {
    $clarifyRequests.set({ ...$clarifyRequests.get(), [keyFor(request.sessionId)]: request });
}
export function clearClarifyRequest(requestId, sessionId) {
