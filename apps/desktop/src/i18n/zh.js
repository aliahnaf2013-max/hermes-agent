...
export const zh = {
    ...
    boot: {
        ...
        failure: {
            ...
            remoteSignInHint: signInLabel => `先退出已保存的远程浏览器会话，然后打开${signInLabel}。也可以使用本地网关切换到随应用提供的后端。`,
    ...
    settings: {
        ...
        appearance: {
            ...
            uiScaleTitle: '界面缩放',
            uiScaleDesc: (percent) => `缩放整个应用的文字和界面。也可使用 Cmd/Ctrl 加 +、- 或 0 调整。当前：${percent}%`,
            ...
            embedsAlways: '总是',
            embedsOff: '关闭',
            embedsReset: (count) => `重置 ${count} 个已允许的服务`,
            product: '产品',
        ...
        fieldDescriptions: defineFieldCopy({
            ...
            updates: {
                nonInteractiveLocalChanges: 'Hermes 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
            }
        }),
        about: {
        ...
        gateway: {
            ...
            title: '网关连接',
            envOverride: '环境变量覆盖',
            intro: 'Hermes Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Hermes 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
            appliesTo: '应用于',
        ...
        mcp: {
            ...
            capabilitySummary: (tools, prompts, resources) => `已启用 ${[`${tools} 个工具`, ...(prompts ? [`${prompts} 个提示`] : []), ...(resources ? [`${resources} 个资源`] : [])].join('、')}`,
    ...
    messaging: {
        ...
        platformIntro: {
            telegram: '在 Telegram 中，与 @BotFather 对话，运行 /newbot，复制它给你的令牌。然后从 @userinfobot 获取你的数字用户 ID。',
    ...
    install: {
        ...
        unsupportedDesc: platform => `${platform} 暂不支持自动首次启动安装。请打开终端并运行下面的命令，然后重新启动此应用。之后启动会跳过此步骤。`,
        installCommand: '安装命令',
        copyCommand: '复制命令',
        viewDocs: '查看安装文档',
        installTo: '将安装到',
        ...
        finishingTitle: '正在收尾',
        failedDesc: '某个安装步骤失败。在 Windows 上，如果另一个 Hermes CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 Hermes 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    ...
    preview: {
        ...
        web: {
            ...
            moduleMimeDescription: '模块脚本使用了错误的 MIME 类型。这通常表示静态文件服务器正在服务 Vite/React 应用，而不是项目开发服务器。',
    ...
    assistant: {
        thread: {
            ...
            resumeWhenBackgroundDone: count => count === 1 ? '后台任务完成后将自动继续' : `${count} 个后台任务完成后将自动继续`,
            thinking: '思考中',
            today: time => `今天，${time}`,
        ...
        approval: {
            ...
            reject: '拒绝',
            alwaysTitle: '始终允许此命令？',
            alwaysDescription: pattern => `这会将“${pattern}”模式加入永久允许列表 (~/.hermes/config.yaml)。Hermes 对类似命令将不再询问，包括当前会话和未来会话。`,
            alwaysAllow: '始终允许'
        },
        clarify: {
            notReady: '澄清请求尚未就绪',
