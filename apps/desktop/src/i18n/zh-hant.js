...
export const zhHant = defineLocale({
    ...
    boot: {
        ...
        failure: {
            ...
            remoteSignInHint: signInLabel => `先登出已儲存的遠端瀏覽器工作階段，然後開啟${signInLabel}。使用本機閘道可切換至內建後端。`,
    ...
    settings: {
        ...
        appearance: {
            ...
            uiScaleTitle: '介面縮放',
            uiScaleDesc: (percent) => `縮放整個應用程式的文字與介面。也可使用 Cmd/Ctrl 加 +、- 或 0 調整。目前：${percent}%`,
            ...
            embedsAlways: '一律',
            embedsOff: '關閉',
            embedsReset: (count) => `重設 ${count} 個已允許的服務`,
            product: '產品',
        ...
        fieldDescriptions: defineFieldCopy({
            ...
            updates: {
                nonInteractiveLocalChanges: 'Hermes 從應用程式內更新自身時，保留本機原始碼變更（stash）或丟棄（discard）。終端機更新一律會詢問。'
            }
        }),
        about: {
        ...
        gateway: {
            ...
            title: '閘道連線',
            envOverride: '環境變數覆寫',
            intro: 'Hermes Desktop 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 Hermes 後端，請使用遠端閘道。在下方按設定檔指定各自的遠端主機。',
            appliesTo: '套用至',
            allProfiles: '全部設定檔',
            ...
            remoteTitle: '遠端閘道',
            remoteDesc: '將此桌面殼層連線至遠端 Hermes 後端。託管閘道使用 OAuth 或帳號密碼；自託管閘道也可使用工作階段 Token。',
        ...
        mcp: {
            ...
            remove: '移除',
            saveServer: '儲存伺服器',
            capabilitySummary: (tools, prompts, resources) => `已啟用 ${[`${tools} 個工具`, ...(prompts ? [`${prompts} 個提示`] : []), ...(resources ? [`${resources} 個資源`] : [])].join('、')}`,
    ...
    updates: {
        ...
        guiSkewBody: '後端已更新，但此桌面應用程式套件未變更。請更新或重新安裝 Hermes 桌面應用程式（你的 AppImage / .deb / .rpm）以保持一致。',
        copy: '複製',
        copied: '已複製',
        done: '完成',
    ...
    install: {
        ...
        finishingTitle: '正在收尾',
        failedDesc: '某個安裝步驟失敗。在 Windows 上，如果另一個 Hermes CLI 或桌面執行個體正在執行，可能會出現這種情況。請停止正在執行的 Hermes 執行個體後重試。可查看下方的詳細資訊或 desktop 記錄中的完整記錄。',
    ...
    preview: {
        ...
        web: {
            ...
            moduleMimeDescription: '模組指令碼使用了錯誤的 MIME 類型。這通常表示靜態檔案伺服器正在服務 Vite/React 應用程式，而不是專案開發伺服器。',
    ...
    assistant: {
        thread: {
            ...
            resumeWhenBackgroundDone: count => count === 1 ? '背景工作完成後將自動繼續' : `${count} 個背景工作完成後將自動繼續`,
            thinking: '思考中',
            today: time => `今天，${time}`,
        ...
        approval: {
            ...
            reject: '拒絕',
            alwaysTitle: '一律允許此指令？',
            alwaysDescription: pattern => `這會將「${pattern}」模式加入永久允許清單（~/.hermes/config.yaml）。Hermes 對類似指令將不再詢問，包括目前工作階段和未來工作階段。`,
            alwaysAllow: '一律允許'
        },
        clarify: {
            notReady: '澄清請求尚未就緒',
