...
const readonlyAtom = (atomLike) => atomLike;
const readViewport = () => ({
    width: typeof window === 'undefined' ? 0 : window.innerWidth,
    height: typeof window === 'undefined' ? 0 : window.innerHeight,
    narrow: $narrowViewport.get()
});
const $viewport = atom(readViewport());
...
export const host = {
    state: {
        ...
        activeSessionId: readonlyAtom($activeSessionId),
        /** Active workspace cwd ('' when detached). */
        cwd: readonlyAtom($currentCwd),
        /** Gateway socket state: 'idle' | 'connecting' | 'open' | …. */
        gateway: readonlyAtom($gatewayState),
        /** Current main model slug. */
        model: readonlyAtom($currentModel),
        /** Profile the live gateway is routed to. */
        profile: readonlyAtom($activeGatewayProfile),
        /** Window geometry ({ width, height, narrow }). */
        viewport: readonlyAtom($viewport)
    },
...
export { PALETTE_AREA } from '@/app/command-palette/contrib';
export { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@/app/routes';
...
export { ContextMenu, ContextMenuContent, ContextMenuItem, ContextMenuSeparator, ContextMenuTrigger } from '@/components/ui/context-menu';
...
export { KEYBINDS_AREA } from '@/lib/keybinds/actions';
