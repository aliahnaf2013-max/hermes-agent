...
import { Activity, Archive, BarChart3, ChevronLeft, ChevronRight, Clock, Cpu, Download, Egg, GitBranch, Globe, Info, KeyRound, Layers3, MessageCircle, Monitor, Moon, Package, Palette, PawPrint, Plus, RefreshCw, Settings, Settings2, SlidersHorizontal, Starmap, Sun, Terminal, Users, Wrench, Zap } from '@/lib/icons';
...
import { AGENTS_ROUTE, ARTIFACTS_ROUTE, COMMAND_CENTER_ROUTE, CRON_ROUTE, MESSAGING_ROUTE, NEW_CHAT_ROUTE, PROFILES_ROUTE, sessionRoute, SETTINGS_ROUTE, SKILLS_ROUTE, STARMAP_ROUTE } from '../routes';
...
const PAGE_PARENTS = { 'install-theme': 'theme' };
...
const rankGroups = (groups, search) => {
...
const paletteValue = (item) => `${item.label}\u0001${item.id}`;
...
const toSessionEntry = (session) => ({
    id: session.id,
    preview: session.preview ?? undefined,
    title: sessionTitle(session)
});
const NON_CONFIG_SETTINGS = [
    {
        icon: Zap,
...
function themeSupportsMode(name, target) {
    if (!isUserTheme(name)) {
        return true;
    }
...
export function CommandPalette() {
    ...
    const settingsSectionLabel = useCallback((section) => t.settings.sections[section.id] ?? section.label, [t.settings.sections]);
    const configFieldLabel = useCallback((key) => fieldCopyForSchemaKey(t.settings.fieldLabels, key) ??
        fieldCopyForSchemaKey(FIELD_LABELS, key) ??
        prettyName(key.split('.').pop() ?? key), [t.settings.fieldLabels]);
    const contributedItems = usePaletteContributions();
    const baseGroups = useMemo(() => {
        const settingsTab = (tab) => `${SETTINGS_ROUTE}?tab=${tab}`;
        ...
        const branchGroup = worktrees.length > 0
            ? [
                {
                    heading: cc.branches,
    ...
    const searchGroups = useMemo(() => {
        ...
        const fieldItems = SECTIONS.flatMap(section => section.keys.map(key => ({
            icon: section.icon,
            id: `field-${key}`,
    ...
    return (_jsx(DialogPrimitive.Root, { onOpenChange: setCommandPaletteOpen, open: open, children: _jsxs(DialogPrimitive.Portal, { children: [_jsx(DialogPrimitive.Overlay, { className: "fixed inset-0 z-[200]" }), _jsxs(DialogPrimitive.Content, { "aria-describedby": undefined, className: cn(HUD_POSITION, HUD_SURFACE, 'z-[210] w-[min(34rem,calc(100vw-2rem))] overflow-hidden duration-150 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:slide-in-from-top-2 data-[state=open]:zoom-in-95'), children: [_jsx(DialogPrimitive.Title, { className: "sr-only", children: t.commandCenter.paletteTitle }), _jsxs(Command, { className: "bg-transparent", loop: true, shouldFilter: false, children: [activePage && (_jsxs("button", { className: "flex w-full items-center gap-1.5 border-b border-border px-3 py-1.5 text-left text-xs text-muted-foreground transition-colors hover:text-foreground", onClick: goBack, type: "button", children: [_jsx(ChevronLeft, { className: "size-3.5" }), _jsx("span", { children: t.commandCenter.back }), _jsx("span", { className: "text-muted-foreground/50", children: "/" }), _jsx("span", { className: "font-medium text-foreground", children: activePage.title })] })), _jsx(CommandInput, { className: HUD_TEXT, onKeyDown: event => {
                                        if (!activePage) {
                                            return;
                                        }
                                    ...
                                    }, onValueChange: setSearch, placeholder: placeholder, right: page === 'pets' ? _jsx(PetInlineToggle, {}) : undefined, value: search }), _jsx(CommandList, { className: "dt-portal-scrollbar max-h-[min(20rem,56vh)]", children: page === 'pets' ? (_jsx(PetPalettePage, { onGenerate: () => {
                                            closeCommandPalette();
                                            openPetGenerate();
                                        }, search: search })) : page === 'install-theme' ? (_jsx(MarketplaceThemePage, { onPickTheme: setTheme, search: search })) : (_jsxs(_Fragment, { children: [visibleGroups.length === 0 && (_jsx("div", { className: "py-6 text-center text-sm text-muted-foreground", children: t.commandCenter.noResults })), visibleGroups.map((group, index) => (_jsx(CommandGroup, { className: HUD_HEADING, heading: group.heading, children: group.items.map(item => {
                                                    const Icon = item.icon;
                                                    const combo = item.action ? bindings[item.action]?.[0] : undefined;
                                                    return (_jsxs(CommandItem, { className: cn(HUD_ITEM, HUD_TEXT), keywords: item.keywords, onSelect: () => handleSelect(item), value: paletteValue(item), children: [_jsx(Icon, { className: "size-3.5 shrink-0 text-muted-foreground" }), _jsx("span", { className: "truncate", children: item.label }), combo && _jsx(KbdCombo, { className: "ml-auto opacity-55", combo: combo, size: "sm" }), item.to && (_jsx(ChevronRight, { className: cn('size-3.5 shrink-0 text-muted-foreground/70', !combo && 'ml-auto') }))] }, item.id));
                                                }) }, group.heading ?? `palette-group-${index}`)))] })) })] })] })] }) }));
}
