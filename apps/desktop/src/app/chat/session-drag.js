...
import { rectContains, slotBefore, snapshotStrips, snapshotZones, startDragSession, subZonePosition } from '@/components/pane-shell/tree/renderer/drag-session';
import { $layoutTree, $treeDragging, revealTreePane, SESSION_TILE_DRAG } from '@/components/pane-shell/tree/store';
import { openSessionTile } from '@/store/session-states';
import { requestComposerInsertRefs } from './composer/focus';
import { sessionInlineRef } from './composer/inline-refs';
const snapRect = (el) => {
 ...
 */
export function startSessionDrag(payload, e) {
    let zones = [];
    let strips = [];
    let surfaces = [];
    let composers = [];
    let zoneHost = new Map();
