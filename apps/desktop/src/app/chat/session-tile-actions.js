...
import { applyBranchVisibility, applyReloadOptimistic, applyRewindOptimistic, finalizeInterruptedMessages, planEdit, planReload, planRestore, runRewindSubmit } from '../session/hooks/use-prompt-actions/rewind';
import { useSubmitPrompt } from '../session/hooks/use-prompt-actions/submit';
export function useSessionTileActions({ runtimeId, scope, storedSessionId }) {
    const { t } = useI18n();
    ...
    const update = useCallback((updater) => sessionTileDelegate()?.updateSession(runtimeIdRef.current, updater), []);
    ...
    const steerPrompt = useCallback(async (rawText) => {
        ...
        try {
            const result = await requestGateway('session.steer', {
    ...
    const submitRewind = useCallback((text, truncateOrdinal, interruptFirst) => runRewindSubmit(requestGateway, runtimeIdRef.current, text, truncateOrdinal, interruptFirst), [requestGateway]);
    const reloadFromMessage = useCallback(async (parentId) => {
        ...
        try {
            await requestGateway('prompt.submit', { session_id: runtimeIdRef.current, text: plan.text, truncate_before_user_ordinal: plan.truncateOrdinal }, PROMPT_SUBMIT_REQUEST_TIMEOUT_MS);
        }
        catch (err) {
    ...
    const handleThreadMessagesChange = useCallback((nextMessages) => update(state => applyBranchVisibility(state, nextMessages)), [update]);
