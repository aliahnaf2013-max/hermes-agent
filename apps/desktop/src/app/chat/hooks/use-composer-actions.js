...
import { addComposerAttachment, removeComposerAttachment, setComposerTerminalSelection } from '@/store/composer';
import { notify, notifyError } from '@/store/notifications';
const IMAGE_EXTENSION_PATTERN = /\.(png|jpe?g|gif|webp|bmp|tiff?|svg|ico)$/i;
const BLOB_MIME_EXTENSION = {
    'image/bmp': '.bmp',
    'image/gif': '.gif',
    'image/jpeg': '.jpg',
    'image/png': '.png',
 ...
 */
export async function attachmentPreviewDataUrl(filePath) {
    try {
 ...
 */
export function extractDroppedFiles(transfer) {
    const result = [];
    ...
    try {
        ...
        if (internalRaw) {
            ...
            const positiveInt = (value) => (typeof value === 'number' && value > 0 ? Math.floor(value) : undefined);
    ...
    const pushNativeEntry = (file, isDirectory) => {
 ...
 */
export function partitionDroppedFiles(candidates) {
    const osDrops = [];
    const inAppRefs = [];
...
const MAIN_ACTIONS_SCOPE = {
    add: addComposerAttachment,
    remove: removeComposerAttachment,
    target: 'main'
};
export function useComposerActions({ activeSessionId, currentCwd, requestGateway, scope = MAIN_ACTIONS_SCOPE }) {
    const { t } = useI18n();
    const copy = t.desktop;
    /** Add to this scope's composer and focus it. All sidebar/picker/drop
     *  attach paths funnel through here. */
    const attachToMain = useCallback((attachment) => {
        scope.add(attachment);
        requestComposerFocus(scope.target);
    }, [scope]);
    const addTextToDraft = useCallback((text) => {
        requestComposerInsert(text, { mode: 'block' });
    }, []);
    const addTerminalSelectionAttachment = useCallback((text, label = 'selection') => {
    ...
    const addContextRefAttachment = useCallback((refText, label, detail) => {
        const kind = refText.startsWith('@folder:')
            ? 'folder'
    ...
    const pickContextPaths = useCallback(async (kind) => {
    ...
    const insertContextPathInlineRef = useCallback((path, isDirectory = false) => {
        if (!path) {
            return false;
        }
    ...
    const attachContextFilePath = useCallback((filePath) => {
        if (!filePath) {
            return false;
        }
    ...
    const attachImagePath = useCallback(async (filePath) => {
        if (!filePath) {
            return false;
        }
    ...
    const attachImageBlob = useCallback(async (blob) => {
        if (blob.size === 0) {
            return false;
        }
    ...
    const pasteClipboardImage = useCallback(async ({ silent = false } = {}) => {
        try {
    ...
    const attachContextFolderPath = useCallback((folderPath) => {
        if (!folderPath) {
            return false;
        }
    ...
    const attachDroppedItems = useCallback(async (candidates) => {
