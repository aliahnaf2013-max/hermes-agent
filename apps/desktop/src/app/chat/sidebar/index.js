...
import { ARTIFACTS_ROUTE, MESSAGING_ROUTE, SIDEBAR_NAV_AREA, SKILLS_ROUTE } from '../../routes';
...
import { overlayLiveLanes, overlayLivePreviews, PROJECT_PREVIEW_COUNT, ProjectBackRow, ProjectMenu, projectTreeCwd, sessionRecency as sessionTime, sortProjectsForOverview, StartWorkButton, useRepoWorktreeMap } from './projects';
...
const SIDEBAR_NAV = [
    {
        id: 'new-session',
        label: '',
        icon: props => _jsx(Codicon, { name: "robot", ...props }),
        action: 'new-session'
    },
    {
        id: 'skills',
        label: '',
        icon: props => _jsx(Codicon, { name: "symbol-misc", ...props }),
        route: SKILLS_ROUTE
    },
...
const HEADER_ACTION_BTN = 'text-(--ui-text-tertiary) opacity-0 transition-opacity hover:bg-(--ui-control-hover-background) hover:text-foreground group-hover/section:opacity-100 focus-visible:opacity-100';
...
const HEADER_NAV_BTN = 'text-(--ui-text-tertiary) opacity-70 transition-opacity hover:bg-(--ui-control-hover-background) hover:text-foreground hover:opacity-100 focus-visible:opacity-100';
// FTS results cover sessions that aren't in the loaded page; synthesize a
// minimal SessionInfo so they render in the same row component (resume works
// by id; the snippet stands in for the preview).
function searchResultToSession(result) {
...
export function ChatSidebar({ currentView, onNavigate, onLoadMoreSessions, onLoadMoreProfileSessions, onLoadMoreMessaging, onResumeSession, onDeleteSession, onArchiveSession, onBranchSession, onNewSessionInWorkspace, onNewSessionSplit, onManageCronJob, onTriggerCronJob }) {
    const { t } = useI18n();
    const s = t.sidebar;
    ...
    const contributedNav = useMemo(() => navContributions.flatMap(c => {
        const data = c.data;
        ...
        return [
            {
                id: c.id,
                label: data.label,
                icon: (props) => _jsx(Codicon, { name: codicon, ...props }),
                route: data.path
            }
        ];
    }), [navContributions]);
    ...
    const visibleSessions = useMemo(() => (showAllProfiles ? sessions : sessions.filter(s => normalizeProfileKey(s.profile) === profileScope)), [sessions, showAllProfiles, profileScope]);
    ...
    const sortedSessions = useMemo(() => [...visibleSessions].sort((a, b) => (b.started_at || 0) - (a.started_at || 0)), [visibleSessions]);
    ...
    const unpinnedAgentSessions = useMemo(() => sortedSessions.filter(s => !pinnedRealIdSet.has(s.id)), [sortedSessions, pinnedRealIdSet]);
    useEffect(() => {
        const next = resolveManualSessionOrderIds(unpinnedAgentSessions.map(s => s.id), agentOrderIds, agentOrderManual);
    ...
    const agentSessions = useMemo(() => (agentOrderManual ? orderByIds(unpinnedAgentSessions, s => s.id, agentOrderIds) : unpinnedAgentSessions), [unpinnedAgentSessions, agentOrderIds, agentOrderManual]);
    ...
    const orderRepos = useCallback((repos) => orderByIds(repos, parent => parent.id, workspaceParentOrderIds).map(parent => ({
        ...parent,
    ...
    const projectModel = useMemo(() => {
        ...
        const sorted = sortProjectsForOverview(projectTree
            .filter(node => !(node.isAuto && dismissed.has(node.id)))
            .map(project => ({ ...project, repos: orderRepos(project.repos) })), activeProjectId);
    ...
    const overviewEnteredProject = projectsActive && projectScope !== ALL_PROJECTS
    ...
    const [enteredProjectTree, setEnteredProjectTree] = useState(null);
    useEffect(() => {
    ...
    const enteredProject = useMemo(() => {
        ...
        const hydrated = enteredProjectTree && enteredProjectTree.id === overviewEnteredProject.id
            ? enteredProjectTree
    ...
    const enteredProjectContent = useMemo(() => (enteredProject ? overlayLiveLanes(enteredProject, agentSessions, removedSessionIds) : undefined), [enteredProject, agentSessions, removedSessionIds]);
    const scopedRepoPaths = useMemo(() => enteredProject ? enteredProject.repos.map(repo => repo.path).filter((path) => Boolean(path)) : [], [enteredProject]);
    ...
    const prevWorkingIdsRef = useRef(workingSessionIds);
    useEffect(() => {
    ...
    const lastProjectCwdSyncRef = useRef(null);
    const syncProjectCwd = useCallback((project) => {
    ...
    const overviewPreviews = useMemo(() => overlayLivePreviews(projectOverview ?? [], agentSessions, projects, PROJECT_PREVIEW_COUNT, removedSessionIds), [projectOverview, agentSessions, projects, removedSessionIds]);
    const onEnterProject = useCallback((id) => {
    ...
    const sessionsLabel = inProject && enteredProject ? enteredProject.label : worktreeGroupingActive ? s.projects.sectionLabel : s.sessions;
    ...
    const projectsSkeletonVisible = worktreeGroupingActive &&
        projectTreeLoading &&
        !projectOverview?.length &&
        !(inProject && (enteredProject?.sessionCount ?? 0) > 0);
    const runKeyedLoad = useCallback((key, load, setPending) => {
        if (!load) {
            return;
        }
    ...
    const loadMoreForProfileGroup = useCallback((profile) => runKeyedLoad(profile, onLoadMoreProfileSessions, setProfileLoadMorePending), [onLoadMoreProfileSessions, runKeyedLoad]);
    const loadMoreForMessaging = useCallback((platform) => runKeyedLoad(platform, onLoadMoreMessaging, setMessagingLoadMorePending), [onLoadMoreMessaging, runKeyedLoad]);
    // Reveal another batch of a platform's rows; fetch from the backend too if we
    // run past what's loaded and more remain on disk.
    const revealMoreMessaging = (platform, loaded, hasMore) => {
    ...
    const knownSessionTotal = Math.max(showAllProfiles ? sessionsTotal : (scopedProfileTotal ?? loadedSessionCount), loadedSessionCount);
    ...
    const activeRepoTrees = useMemo(() => (agentProjectTree ? agentProjectTree.flatMap(project => project.repos) : []), [agentProjectTree]);
    const recentsVirtualizes = !displayAgentGroups?.length && !agentProjectTree?.length && displayAgentSessions.length >= VIRTUALIZE_THRESHOLD;
    ...
    useEffect(() => {
        ...
        const nextParents = reconcileOrderIds(activeRepoTrees.map(parent => parent.id), workspaceParentOrderIds);
        if (!sameIds(nextParents, workspaceParentOrderIds)) {
            setSidebarWorkspaceParentOrderIds(nextParents);
        }
        const nextWorktrees = reconcileOrderIds(activeRepoTrees.flatMap(parent => parent.groups.map(group => group.id)), workspaceOrderIds);
    ...
    const reorderProjects = (ids) => setSidebarProjectOrderIds(ids);
    ...
    const reorderPinned = (ids) => setPinnedSessionOrder(ids.map(id => {
        const session = sessionByAnyId.get(id);
        return session ? sessionPinId(session) : id;
    }));
    return (_jsxs(Sidebar, { className: cn(
        ...
        // paints itself fully.
        'relative h-full min-w-0 overflow-hidden border-t-0 border-b-0 text-foreground transition-none', panesFlipped ? 'border-l border-r-0' : 'border-r border-l-0', 'border-(--sidebar-edge-border) bg-(--ui-sidebar-surface-background) opacity-100'), collapsible: "none", children: [_jsxs(SidebarContent, { className: "gap-0 overflow-hidden bg-transparent px-2.5", children: [_jsx(SidebarGroup, { className: "shrink-0 p-0 pb-2 pt-[calc(var(--titlebar-height)+0.375rem)]", children: _jsx(SidebarGroupContent, { children: _jsx(SidebarMenu, { className: "gap-px", children: [...SIDEBAR_NAV, ...contributedNav].map(item => {
                                    ...
                                    const button = (_jsxs(SidebarMenuButton, { "aria-disabled": !isInteractive, className: cn(
                                        ...
                                        // thread.tsx.
                                        'flex h-7 w-full justify-start gap-2 rounded-md border border-transparent px-2 text-left text-[0.8125rem] font-medium text-(--ui-text-secondary) transition-colors duration-100 ease-out [-webkit-app-region:no-drag] hover:bg-(--ui-control-hover-background) hover:text-foreground hover:transition-none', active &&
                                            'border-(--ui-stroke-tertiary) bg-(--ui-control-active-background) text-foreground shadow-none hover:border-(--ui-stroke-tertiary)!', !isInteractive &&
                                            'cursor-default hover:border-transparent hover:bg-transparent hover:text-inherit'), onClick: () => {
                                            ...
                                            onNavigate(item);
                                        }, tooltip: s.nav[item.id] ?? item.label, type: "button", children: [_jsx(item.icon, { className: "size-4 shrink-0 text-[color-mix(in_srgb,currentColor_72%,transparent)]" }), _jsx("span", { className: "min-w-0 flex-1 truncate", children: s.nav[item.id] ?? item.label }), isNewSession && (_jsx(KbdGroup, { className: cn('ml-auto opacity-55', newSessionKbdFlash && 'opacity-100!'), keys: [...NEW_SESSION_KBD], size: "sm" }))] }));
                                    // New session + route-backed pages can open in a split —
                                    // right-click for the directional "Open in split" submenu.
                                    return (_jsx(SidebarMenuItem, { children: isNewSession || item.route ? (_jsxs(ContextMenu, { children: [_jsx(ContextMenuTrigger, { asChild: true, children: button }), _jsx(ContextMenuContent, { "aria-label": s.nav[item.id] ?? item.label, children: _jsx(SplitSubmenu, { kit: CONTEXT_SPLIT_KIT, label: s.row.openInSplit, onSplit: dir => {
                                                            if (isNewSession) {
                                ...
                                }) }) }) }), showSessionSections && (_jsx("div", { className: "shrink-0 px-2 pb-1 pt-1", children: _jsx(SearchField, { "aria-label": s.searchAria, inputRef: searchInputRef, onChange: setSearchQuery, placeholder: s.searchPlaceholder, value: searchQuery }) })), showSessionSections && (_jsxs("div", { className: cn('flex min-h-0 flex-1 flex-col pb-1.75', SCROLL_Y), children: [trimmedQuery && (_jsx(SidebarSessionsSection, { activeSessionId: activeSidebarSessionId, contentClassName: cn('flex min-h-0 flex-1 flex-col gap-px pb-1.75', SCROLL_Y), emptyState: searchPending ? (_jsx(SidebarSessionSkeletons, {})) : (_jsx("div", { className: "wrap-anywhere grid min-h-24 place-items-center rounded-lg px-2 text-center text-xs text-(--ui-text-tertiary)", children: s.noMatch(trimmedQuery) })), label: s.results, labelMeta: String(searchResults.length), onArchiveSession: onArchiveSession, onBranchSession: onBranchSession, onDeleteSession: onDeleteSession, onResumeSession: onResumeSession, onToggle: () => undefined, onTogglePin: pinSession, open: true, pinned: false, rootClassName: "min-h-32 flex-1 overflow-hidden p-0", sessions: searchResults, workingSessionIdSet: workingSessionIdSet })), !trimmedQuery && (_jsx(SidebarSessionsSection, { activeSessionId: activeSidebarSessionId, contentClassName: cn('flex max-h-44 flex-col gap-px rounded-lg pb-2 pt-1', GROUP_BODY), dndSensors: dndSensors, emptyState: _jsx(SidebarPinnedEmptyState, {}), label: s.pinned, onArchiveSession: onArchiveSession, onBranchSession: onBranchSession, onDeleteSession: onDeleteSession, onReorderSessions: reorderPinned, onResumeSession: onResumeSession, onToggle: () => setSidebarPinsOpen(!pinsOpen), onTogglePin: unpinSession, open: pinsOpen, pinned: true, rootClassName: "shrink-0 p-0 pb-1", sessions: pinnedSessions, sortable: pinnedSessions.length > 1, workingSessionIdSet: workingSessionIdSet })), !trimmedQuery && (_jsx(SidebarSessionsSection, { activeProjectId: activeProjectId, activeSessionId: activeSidebarSessionId, collapsible: !inProject, contentClassName: cn('flex min-h-0 flex-1 flex-col pb-1.75', SCROLL_Y, 
                                // Separate profile sections clearly in the ALL view; rows inside
                                // each group keep their own tight gap-px rhythm.
                                showAllProfiles ? 'gap-3' : 'gap-px', 
                                // Flatten into the single scroll when compact — unless this is the
                                // virtualized long list, which must keep its own scroller.
                                !recentsVirtualizes && COMPACT_FLAT), dndSensors: dndSensors, emptyState: showSessionSkeletons ? (_jsx(SidebarSessionSkeletons, {})) : (_jsx("div", { className: "grid min-h-16 place-items-center rounded-lg px-2 text-center text-xs text-(--ui-text-tertiary)", children: inProject ? s.projectEmpty : pinnedSessions.length > 0 ? s.allPinned : s.noSessions })), footer: 
                                ...
                                !showAllProfiles && !agentsGrouped && !showSessionSkeletons && hasMoreSessions ? (_jsx(SidebarLoadMoreRow, { loading: sessionsLoading || recentsLoadMorePending, onClick: () => void onLoadMoreRecents(), 
                                ...
                                }), !trimmedQuery && !worktreeGroupingActive && cronJobs.length > 0 && (_jsx(SidebarCronJobsSection, { jobs: cronJobs, label: s.cronJobs, onManageJob: onManageCronJob, onOpenRun: onResumeSession, onToggle: () => setSidebarCronOpen(!cronOpen), onTriggerJob: onTriggerCronJob, open: cronOpen }))] })), !showSessionSections && _jsx(SidebarBlankState, { onNewProject: openProjectCreate }), _jsx("div", { className: "shrink-0 px-0.5 pb-1 pt-0.5", children: _jsx(ProfileRail, {}) })] }), _jsx(ProjectDialog, {})] }));
}
