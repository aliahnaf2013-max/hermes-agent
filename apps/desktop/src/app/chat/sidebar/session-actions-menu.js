...
const DROPDOWN_KIT = { Item: DropdownMenuItem, split: DROPDOWN_SPLIT_KIT };
const CONTEXT_KIT = { Item: ContextMenuItem, split: CONTEXT_SPLIT_KIT };
function useSessionActions({ sessionId, title, pinned = false, profile, onPin, onBranch, onArchive, onDelete }) {
    ...
    const renderItems = (kit) => (_jsxs(_Fragment, { children: [renderMenuItem(kit.Item, pinItem), _jsx(CopyButton, { appearance: kit.Item === DropdownMenuItem ? 'menu-item' : 'context-menu-item', disabled: !sessionId, errorMessage: r.copyIdFailed, iconClassName: "size-3.5 text-current", label: r.copyId, onCopyError: err => notifyError(err, r.copyIdFailed), text: sessionId }, r.copyId), _jsx(SplitSubmenu, { close: kit.close, disabled: !sessionId, kit: kit.split, label: r.openInSplit, onSplit: dir => openSessionTile(sessionId, dir) }, "split"), items.map(spec => renderMenuItem(kit.Item, spec))] }));
...
export function SessionActionsMenu({ children, align = 'end', sideOffset = 6, ...actions }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs(DropdownMenu, { onOpenChange: setOpen, open: open, children: [_jsx(DropdownMenuTrigger, { asChild: true, children: children }), _jsx(DropdownMenuContent, { align: align, "aria-label": t.sidebar.row.actionsFor(actions.title), className: "w-40", sideOffset: sideOffset, children: renderItems({ ...DROPDOWN_KIT, close: () => setOpen(false) }) })] }), renameDialog] }));
}
export function SessionContextMenu({ children, ...actions }) {
    const { t } = useI18n();
    const { renameDialog, renderItems } = useSessionActions(actions);
    return (_jsxs(_Fragment, { children: [_jsxs(ContextMenu, { children: [_jsx(ContextMenuTrigger, { asChild: true, children: children }), _jsx(ContextMenuContent, { "aria-label": t.sidebar.row.actionsFor(actions.title), className: "w-40", children: renderItems({ ...CONTEXT_KIT, close: () => undefined }) })] }), renameDialog] }));
}
