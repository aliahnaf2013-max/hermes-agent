...
import { COMPOSER_FADE_BACKGROUND, slashArgStage } from './composer-utils';
...
import { composerPlainText, deleteChipBeforeCaret, deleteSelectionInEditor, insertPlainTextAtCaret, normalizeComposerEditorDom, RICH_INPUT_SLOT } from './rich-editor';
...
export function ChatBar({ busy, cwd, disabled, focusKey, gateway, maxRecordingSeconds = 120, queueSessionKey, sessionId, state, onCancel, onAddUrl, onAttachDroppedItems, onAttachImageBlob, onPasteClipboardImage, onPickFiles, onPickFolders, onPickImages, onRemoveAttachment, onSteer, onSubmit: onSubmitProp, onTranscribeAudio }) {
    ...
    const { dockProximity, dragging, handleComposerToggle, onComposerGesturePointerDown, popoutAllowed, popoutPosition, poppedOut } = useComposerPopout({ composerRef });
    ...
    const { activeQueueSessionKeyRef, clearDraft, draftRef, editorRef, focusInput, hasText, insertInlineRefs, insertText, isHelpHint, isSteerableText, loadIntoComposer, requestMainFocus, sessionIdRef, setComposerText, stashAt } = useComposerDraft({ activeQueueSessionKey, focusKey, inputDisabled, queueEditRef, sessionId });
    ...
    const { beginQueuedEdit, drainNextQueued, editingQueuedPrompt, exitQueuedEdit, queueCurrentDraft, queueEdit, queuedPrompts, sendQueuedNow, stepQueuedEdit } = useComposerQueue({
    ...
    const { argStageEmpty, closeTrigger, commitTypedSlashDirective, refreshTrigger, replaceTriggerWithChip, setTriggerActive, trigger, triggerActive, triggerItems, triggerKeyConsumedRef, triggerLoading } = useComposerTrigger({ at, draftRef, editorRef, requestMainFocus, setComposerText, slash });
    ...
    const flushRafRef = useRef(undefined);
    const flushEditorToDraft = (editor) => {
    ...
    const scheduleFlushEditorToDraft = (editor) => {
        if (flushRafRef.current !== undefined) {
            return;
        }
    ...
    const { dragActive, handleDragEnter, handleDragLeave, handleDragOver, handleDrop, handleInputDragOver, handleInputDrop } = useComposerDrop({ cwd, insertInlineRefs, onAttachDroppedItems, requestMainFocus });
    ...
    const { handleBranchOff, handleConvertBranch, handleListBranches, handleSwitchBranch, openInWorktree } = useComposerBranch({ clearDraft, cwd, draftRef });
    ...
    const { conversation, dictate, endConversation, handleToggleAutoSpeak, startConversation, voiceActivityState, voiceConversationActive, voiceStatus } = useComposerVoice({
        busy,
        clearDraft,
        disabled,
        focusInput,
    ...
    const contextMenu = (_jsx(ContextMenu, { onInsertText: insertText, onOpenUrlDialog: openUrlDialog, onPasteClipboardImage: onPasteClipboardImage, onPickFiles: onPickFiles, onPickFolders: onPickFolders, onPickImages: onPickImages, state: state }));
    const controls = (_jsx(ComposerControls, { autoSpeak: autoSpeak, busy: busy, busyAction: busyAction, canSteer: canSteer, canSubmit: canSubmit, compactModelPill: poppedOut, conversation: {
        ...
        }, disabled: disabled, hasComposerPayload: hasComposerPayload, onDictate: dictate, onSteer: steerDraft, onToggleAutoSpeak: handleToggleAutoSpeak, state: state, voiceStatus: voiceStatus }));
    const input = (_jsxs("div", { className: cn('relative', stacked ? 'w-full' : 'min-w-(--composer-input-inline-min-width) flex-1'), children: [_jsx("div", { "aria-disabled": inputDisabled ? true : undefined, "aria-label": t.composer.message, autoCapitalize: "off", autoCorrect: "off", className: cn('min-h-(--composer-input-min-height) max-h-(--composer-input-max-height) cursor-text overflow-y-auto whitespace-pre-wrap break-words [overflow-wrap:anywhere] bg-transparent pb-1 pr-1 pt-1 leading-normal text-foreground outline-none disabled:cursor-not-allowed', 'empty:before:content-[attr(data-placeholder)] empty:before:text-muted-foreground/60', '**:data-ref-text:cursor-default', stacked && 'pl-3', stacked ? 'w-full' : 'min-w-(--composer-input-inline-min-width) flex-1'), contentEditable: !inputDisabled, "data-placeholder": placeholder, "data-slot": RICH_INPUT_SLOT, onBlur: () => window.setTimeout(closeTrigger, 80), onCompositionEnd: event => {
                ...
                }, onDragOver: handleInputDragOver, onDrop: handleInputDrop, onFocus: () => markActiveComposer(scope.target), onInput: handleEditorInput, onKeyDown: handleEditorKeyDown, onKeyUp: handleEditorKeyUp, onMouseUp: refreshTrigger, onPaste: handlePaste, ref: editorRef, role: "textbox", spellCheck: false, suppressContentEditableWarning: true }), _jsx(ComposerPrimitive.Input, { asChild: true, submitMode: "ctrlEnter", tabIndex: -1, unstable_focusOnScrollToBottom: false, children: _jsx("textarea", { "aria-hidden": true, autoCapitalize: "off", autoComplete: "off", autoCorrect: "off", className: "sr-only", spellCheck: false, tabIndex: -1 }) })] }));
    return (_jsxs(_Fragment, { children: [dragging && poppedOut && (_jsx("div", { "aria-hidden": true, 
                ...
                // a containing block.
                className: "pointer-events-none absolute inset-x-0 bottom-0 z-20 h-32", style: {
                    ...
                    background: 'radial-gradient(64% 130% at 50% 100%, color-mix(in srgb, var(--color-primary) 26%, transparent) 0%, transparent 70%)',
                    // Scaled by --dock-glow-scale (lower in light mode — see styles.css).
                    opacity: `calc(${0.1 + dockProximity * 0.57} * var(--dock-glow-scale, 1))`
                } })), _jsx(ComposerPrimitive.Unstable_TriggerPopoverRoot, { children: _jsxs(ComposerPrimitive.Root, { className: cn('group/composer z-30 overflow-visible rounded-2xl', poppedOut
                        ...
                        : 'absolute bottom-0 left-1/2 w-[min(var(--composer-width),calc(100%-2rem))] max-w-full -translate-x-1/2 pt-2 pb-[var(--composer-shell-pad-block-end)]', dragging && 'cursor-grabbing select-none touch-none'), "data-drag-active": dragActive ? '' : undefined, "data-popped-out": poppedOut ? '' : undefined, "data-slot": "composer-root", "data-status-stack": statusStackVisible ? '' : undefined, "data-thread-scrolled-up": scrolledUp ? '' : undefined, onDragEnter: handleDragEnter, onDragLeave: handleDragLeave, onDragOver: handleDragOver, onDrop: handleDrop, onPointerDown: popoutAllowed ? onComposerGesturePointerDown : undefined, onSubmit: e => {
                        e.preventDefault();
                        if (composingRef.current) {
                            return;
                        }
                        submitDraft();
                    }, ref: composerRef, style: poppedOut
                        ? {
                            bottom: `${popoutPosition.bottom}px`,
                            right: `${popoutPosition.right}px`,
                            // A compact one-sentence width when floating.
                            ['--composer-popout-width']: `${POPOUT_WIDTH_REM}rem`
                        }
                        : undefined, children: [isHelpHint && _jsx(HelpHint, {}), trigger && !argStageEmpty && (_jsx(ComposerTriggerPopover, { activeIndex: triggerActive, items: triggerItems, kind: trigger.kind, loading: triggerLoading, onHover: setTriggerActive, onPick: replaceTriggerWithChip })), _jsx(ComposerStatusStack, { queue: activeQueueSessionKey && queuedPrompts.length > 0 ? (_jsx(QueuePanel, { busy: busy, editingId: queueEdit?.entryId ?? null, entries: queuedPrompts, onDelete: id => {
                                    if (removeQueuedPrompt(activeQueueSessionKey, id) && queueEdit?.entryId === id) {
                                        exitQueuedEdit('cancel');
                                    }
                                }, onEdit: beginQueuedEdit, onSendNow: id => void sendQueuedNow(id) })) : null, sessionId: statusSessionId }), !poppedOut && (_jsx("div", { className: "pointer-events-none absolute inset-0 rounded-[inherit]", style: { background: COMPOSER_FADE_BACKGROUND } })), popoutAllowed && (_jsx("div", { "aria-hidden": true, className: cn('pointer-events-auto absolute inset-0', dragging ? 'cursor-grabbing' : 'cursor-grab'), "data-dragging": dragging ? '' : undefined, "data-slot": "composer-drag-region", onDoubleClick: handleComposerToggle })), _jsx("div", { className: "relative w-full rounded-[inherit]", children: _jsxs("div", { className: cn('group/composer-surface relative z-4 isolate grid grid-rows-[auto_1fr] overflow-hidden rounded-[inherit] border border-[color-mix(in_srgb,var(--dt-composer-ring)_calc(18%*var(--composer-ring-strength)),var(--dt-input))]', COMPOSER_DROP_FADE_CLASS, dragActive && COMPOSER_DROP_ACTIVE_CLASS), "data-slot": "composer-surface", ref: composerSurfaceRef, children: [_jsx("div", { "aria-hidden": true, className: cn('pointer-events-none absolute inset-0 -z-10 rounded-[inherit]', composerFill, composerSurfaceGlass) }), _jsx(CodingStatusRow, { onBranchOff: handleBranchOff, onConvertBranch: handleConvertBranch, onListBranches: handleListBranches, onOpen: toggleReview, onOpenWorktree: openInWorktree, onSwitchBranch: handleSwitchBranch }), _jsxs("div", { className: cn('relative z-1 flex min-h-0 w-full flex-col gap-(--composer-row-gap) overflow-hidden rounded-[inherit] px-(--composer-surface-pad-x) py-(--composer-surface-pad-y) transition-opacity duration-200 ease-out', scrolledUp
                                            ? 'opacity-30 group-hover/composer:opacity-100 group-focus-within/composer-surface:opacity-100'
                                            : 'opacity-100'), "data-slot": "composer-fade", children: [_jsx(ContribSlot, { area: COMPOSER_AREAS.top }), _jsx(VoiceActivity, { state: voiceActivityState }), _jsx(VoicePlaybackActivity, {}), queueEdit && editingQueuedPrompt && (_jsxs("div", { className: "flex items-center justify-between gap-2 rounded-lg border border-[color-mix(in_srgb,var(--dt-composer-ring)_32%,transparent)] bg-accent/18 px-2 py-1", children: [_jsx("div", { className: "min-w-0 text-[0.7rem] text-muted-foreground/88", children: t.composer.editingQueuedInComposer }), _jsxs("div", { className: "flex shrink-0 items-center gap-1", children: [_jsx(Button, { className: "h-6 rounded-md px-2 text-[0.68rem]", onClick: () => exitQueuedEdit('cancel'), type: "button", variant: "ghost", children: t.common.cancel }), _jsx(Button, { className: "h-6 rounded-md px-2 text-[0.68rem]", onClick: () => exitQueuedEdit('save'), type: "button", children: t.common.save })] })] })), attachments.length > 0 && _jsx(AttachmentList, { attachments: attachments, onRemove: onRemoveAttachment }), _jsxs("div", { className: cn('grid w-full', stacked
                                                    ? 'grid-cols-[auto_1fr] gap-(--composer-row-gap) [grid-template-areas:"input_input"_"menu_controls"]'
                                                    : 'grid-cols-[auto_1fr_auto] items-center gap-(--composer-control-gap) [grid-template-areas:"menu_input_controls"]'), children: [_jsxs("div", { className: "flex translate-y-[3px] items-start gap-(--composer-control-gap) self-start [grid-area:menu]", children: [contextMenu, _jsx(ContribSlot, { area: COMPOSER_AREAS.leading })] }), _jsx("div", { className: "min-w-0 [grid-area:input]", children: input }), _jsxs("div", { className: "flex items-center justify-end gap-(--composer-control-gap) [grid-area:controls]", children: [_jsx(ContribSlot, { area: COMPOSER_AREAS.actions }), controls] })] }), _jsx(ContribSlot, { area: COMPOSER_AREAS.bottom })] })] }) })] }) }), _jsx(UrlDialog, { inputRef: urlInputRef, onChange: setUrlValue, onOpenChange: setUrlOpen, onSubmit: submitUrl, open: urlOpen, value: urlValue })] }));
}
export function ChatBarFallback() {
    return (_jsx("div", { className: cn('group/composer absolute bottom-0 left-1/2 z-30 w-[min(var(--composer-width),calc(100%-2rem))] max-w-full -translate-x-1/2 rounded-2xl pt-2 pb-[var(--composer-shell-pad-block-end)]', 'bg-linear-to-b from-transparent to-background/55'), "data-slot": "composer-root", children: _jsx("div", { className: "composer-fallback-surface relative isolate h-(--composer-fallback-height) w-full rounded-[inherit] border border-[color-mix(in_srgb,var(--dt-composer-ring)_calc(18%*var(--composer-ring-strength)),var(--dt-input))]", children: _jsx("div", { "aria-hidden": true, className: cn('pointer-events-none absolute inset-0 -z-10 rounded-[inherit]', composerFill, composerSurfaceGlass) }) }) }));
}
