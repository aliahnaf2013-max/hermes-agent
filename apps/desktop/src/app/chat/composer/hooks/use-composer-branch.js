 ...
 */
export function useComposerBranch({ clearDraft, cwd, draftRef }) {
    ...
    const openInWorktree = useCallback((path) => {
    ...
    const handleBranchOff = useCallback(async (branch, base) => {
    ...
    const handleConvertBranch = useCallback(async (branch, path, isDefault) => {
        if (path?.trim()) {
            openInWorktree(path);
            return;
        }
    ...
    const handleSwitchBranch = useCallback(async (branch) => {
        const repoPath = cwd?.trim();
        if (repoPath) {
            await switchBranchInRepo(repoPath, branch);
        }
