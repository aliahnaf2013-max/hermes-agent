...
import { cloneAttachments, DRAFT_PERSIST_DEBOUNCE_MS, isPendingDraftPersistCurrent } from '../composer-utils';
import { focusComposerInput, markActiveComposer, onComposerFocusRequest, onComposerInsertRefsRequest, onComposerInsertRequest } from '../focus';
 ...
 */
export function useComposerDraft({ activeQueueSessionKey, focusKey, inputDisabled, queueEditRef, sessionId }) {
    const aui = useAui();
    ...
    const pendingDraftPersistRef = useRef(null);
    const draftPersistTimerRef = useRef(undefined);
    ...
    const queueEditStateRef = useRef(queueEditRef.current);
    ...
    const paintDraft = useCallback((next, focus = true) => {
    ...
    const appendExternalText = useCallback((text, mode) => {
    ...
    const stashAt = (scope, text = draftRef.current, attachments = attachmentScope.$attachments.get()) => stashSessionDraft(scope, text, attachments);
    const loadIntoComposer = (text, attachments) => {
