...
import { $queuedPromptsBySession, enqueueQueuedPrompt, MAX_AUTO_DRAIN_ATTEMPTS, migrateQueuedPrompts, promoteQueuedPrompt, removeQueuedPrompt, shouldAutoDrain, updateQueuedPrompt } from '@/store/composer-queue';
import { notify } from '@/store/notifications';
import { cloneAttachments } from '../composer-utils';
 ...
 */
export function useComposerQueue({ activeQueueSessionKey, attachments, busy, clearDraft, draftRef, focusInput, loadIntoComposer, onCancel, onSubmit, queueEditRef, queueSessionKey, sessionId }) {
    const { t } = useI18n();
    ...
    const setQueueEditSnapshot = useCallback((next) => {
    ...
    const drainFailuresRef = useRef(new Map());
    const beginQueuedEdit = (entry) => {
    ...
    const stepQueuedEdit = (direction) => {
        if (!queueEdit) {
            return false;
        }
    ...
    const runDrain = useCallback(async (pickEntry) => {
    ...
    const pickDrainHead = useCallback((entries) => {
    ...
    const sendQueuedNow = useCallback((id) => {
