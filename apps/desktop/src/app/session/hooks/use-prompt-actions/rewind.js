...
import { branchGroupForUser, chatMessageText, textPart } from '@/lib/chat-messages';
 ...
 */
export async function runRewindSubmit(requestGateway, sessionId, text, truncateOrdinal, interruptFirst) {
    ...
    const submit = () => requestGateway('prompt.submit', { session_id: sessionId, text, ...(truncateOrdinal !== undefined && { truncate_before_user_ordinal: truncateOrdinal }) }, PROMPT_SUBMIT_REQUEST_TIMEOUT_MS);
    if (interruptFirst) {
        await interrupt();
    }
    try {
...
export function finalizeInterruptedMessages(messages, streamId) {
    return messages
...
export function planReload(messages, parentId) {
    const parentIndex = parentId ? messages.findIndex(m => m.id === parentId) : messages.length - 1;
    const userBack = parentIndex >= 0 ? [...messages.slice(0, parentIndex + 1)].reverse().findIndex(m => m.role === 'user') : -1;
    if (userBack < 0) {
        return null;
    }
    ...
    const targetAssistant = parentId && messages[parentIndex]?.role === 'assistant'
...
export function applyReloadOptimistic(state, plan) {
...
export function planRestore(messages, messageId, target) {
    const idIndex = messages.findIndex(m => m.id === messageId && m.role === 'user');
    const fallbackIndex = target?.userOrdinal === null || target?.userOrdinal === undefined
        ? -1
    ...
    const truncateOrdinal = target?.userOrdinal === null || target?.userOrdinal === undefined
...
export function planEdit(messages, edited) {
...
export function applyRewindOptimistic(state, sourceIndex, editedMessage) {
    return {
        ...state,
        awaitingResponse: true,
        busy: true,
...
export function applyBranchVisibility(state, next) {
