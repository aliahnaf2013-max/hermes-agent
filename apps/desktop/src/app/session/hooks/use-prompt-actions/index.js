...
import { $composerAttachments, setComposerAttachmentUploadState, updateComposerAttachment } from '@/store/composer';
...
import { applyBranchVisibility, applyReloadOptimistic, applyRewindOptimistic, finalizeInterruptedMessages, planEdit, planReload, planRestore, runRewindSubmit } from './rewind';
import { useSlashCommand } from './slash';
import { useSubmitPrompt } from './submit';
import { blobToDataUrl, delay, friendlyRemoteAttachError, inlineErrorMessage, isSessionNotFoundError, readFileDataUrlForAttach, readImageForRemoteAttach } from './utils';
/**
 ...
 */
export async function uploadComposerAttachment(attachment, opts) {
...
export function usePromptActions({ activeSessionId, activeSessionIdRef, busyRef, branchCurrentSession, createBackendSessionForSend, getRouteToken, handleSkinCommand, openMemoryGraph, refreshSessions, requestGateway, resumeStoredSession, selectedStoredSessionIdRef, startFreshSessionDraft, sttEnabled, updateSessionState }) {
    const { t } = useI18n();
    const copy = t.desktop;
    const appendSessionTextMessage = useCallback((sessionId, role, text) => {
    ...
    const syncAttachmentsForSubmit = useCallback(async (sessionId, attachments, options = {}) => {
    ...
    const eagerlyUploadAttachment = useCallback(async (sessionId, attachment) => {
        const remote = $connection.get()?.mode === 'remote';
    ...
    useEffect(() => {
        ...
        for (const attachment of composerAttachments) {
            const needsUpload = attachment.kind === 'file' &&
            ...
            const task = eagerlyUploadAttachment(activeSessionId, attachment).finally(() => eagerUploadInFlight.current.delete(attachment.id));
    ...
    const handoffSession = useCallback(async (platform, options) => {
    ...
    const submitText = useCallback(async (rawText, options) => {
    ...
    const transcribeVoiceAudio = useCallback(async (audio) => {
        if (!sttEnabled) {
    ...
    const steerPrompt = useCallback(async (rawText) => {
        ...
        try {
            const result = await requestGateway('session.steer', { session_id: sessionId, text });
    ...
    const reloadFromMessage = useCallback(async (parentId) => {
        ...
        try {
            await requestGateway('prompt.submit', { session_id: activeSessionId, text: plan.text, truncate_before_user_ordinal: plan.truncateOrdinal }, PROMPT_SUBMIT_REQUEST_TIMEOUT_MS);
        }
        catch (err) {
    ...
    const submitRewindPrompt = useCallback((sessionId, text, truncateOrdinal, interruptFirst) => runRewindSubmit(requestGateway, sessionId, text, truncateOrdinal, interruptFirst), [requestGateway]);
    const restoreToMessage = useCallback(async (messageId, target) => {
        const sessionId = activeSessionId || activeSessionIdRef.current;
        if (!sessionId) {
    ...
    const editMessage = useCallback(async (edited) => {
        const sessionId = activeSessionId || activeSessionIdRef.current;
        const messages = $messages.get();
        ...
        const isStaleTargetError = (err) => /no longer in session history|not in session history/i.test(err instanceof Error ? err.message : String(err));
        try {
            await submitRewindPrompt(sessionId, plan.text, plan.truncateOrdinal, busyRef.current || $busy.get());
        }
        catch (err) {
    ...
    const handleThreadMessagesChange = useCallback((nextMessages) => {
