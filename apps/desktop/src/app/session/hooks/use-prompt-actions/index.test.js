...
function Harness({ activeSessionIdRef: activeSessionIdRefProp, busyRef, getRouteToken, onReady, onSeedState, openMemoryGraph, refreshSessions, requestGateway, resumeStoredSession, seedMessages, selectedStoredSessionIdRef: selectedStoredSessionIdRefProp, storedSessionId, activeSessionId, createBackendSessionForSend }) {
    const activeSessionIdRef = activeSessionIdRefProp ?? {
        current: activeSessionId === undefined ? RUNTIME_SESSION_ID : activeSessionId
    };
    const selectedStoredSessionIdRef = selectedStoredSessionIdRefProp ?? {
...
describe('usePromptActions /title', () => {
    ...
    it('renames via the session.title RPC (with the runtime id), updates the sidebar store, and refreshes', async () => {
        ...
        const requestGateway = vi.fn(async (method) => (method === 'session.title' ? { pending: false, title: 'New title' } : {}));
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), refreshSessions: refreshSessions, requestGateway: requestGateway }));
    ...
    it('reports the queued state when the session row is not persisted yet', async () => {
        const refreshSessions = vi.fn(async () => undefined);
        const requestGateway = vi.fn(async (method) => (method === 'session.title' ? { pending: true, title: 'Fresh chat' } : {}));
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), refreshSessions: refreshSessions, requestGateway: requestGateway }));
    ...
    it('falls through to the slash worker for a bare /title (show current title)', async () => {
        const refreshSessions = vi.fn(async () => undefined);
        const requestGateway = vi.fn(async () => ({ output: 'Title: Old title' }));
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), refreshSessions: refreshSessions, requestGateway: requestGateway }));
...
describe('usePromptActions slash.exec dispatch payloads', () => {
    ...
    it('submits /goal send directives returned directly by slash.exec instead of rendering no output', async () => {
        ...
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), onSeedState: s => states.push(s), refreshSessions: async () => undefined, requestGateway: requestGateway }));
    ...
    it('restores a degenerate slash payload to the composer instead of losing it', async () => {
        ...
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), refreshSessions: async () => undefined, requestGateway: requestGateway }));
...
describe('usePromptActions desktop slash pickers', () => {
    ...
    it('resumes an exact session id even when it is not in the loaded sidebar cache', async () => {
        ...
        const requestGateway = vi.fn(async () => ({}));
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), refreshSessions: async () => undefined, requestGateway: requestGateway, resumeStoredSession: resumeStoredSession }));
    ...
    it('opens the memory graph overlay for /journey and its aliases instead of hitting the backend', async () => {
        ...
        const requestGateway = vi.fn(async () => ({}));
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), openMemoryGraph: openMemoryGraph, refreshSessions: async () => undefined, requestGateway: requestGateway }));
...
describe('usePromptActions submit / queue drain semantics', () => {
    ...
    it('clears a leftover interrupted flag on a fresh submit (so the new turn streams)', async () => {
        ...
        const requestGateway = vi.fn(async () => ({}));
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), onSeedState: s => seeds.push(s), refreshSessions: async () => undefined, requestGateway: requestGateway }));
    ...
    it('a fromQueue drain sends even when busyRef is still true on the settle edge', async () => {
        ...
        const requestGateway = vi.fn(async () => ({}));
        let handle = null;
        render(_jsx(Harness, { busyRef: busyRef, onReady: h => (handle = h), refreshSessions: async () => undefined, requestGateway: requestGateway }));
    ...
    it('a rejected fromQueue drain returns false (entry stays queued) and a later retry sends it', async () => {
        ...
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), refreshSessions: async () => undefined, requestGateway: requestGateway, storedSessionId: null }));
    ...
    it('rides out a transient "session busy" so the user never sees it (retries, no error bubble)', async () => {
        ...
        expect(seeds.some(s => Array.isArray(s.messages) && s.messages.some(m => m.error))).toBe(false);
    });
...
describe('usePromptActions steerPrompt', () => {
    ...
    it('injects the trimmed text via session.steer and reports acceptance on a queued status', async () => {
        const requestGateway = vi.fn(async () => ({ status: 'queued' }));
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), refreshSessions: async () => undefined, requestGateway: requestGateway }));
    ...
    it('reports rejection (so the caller queues) when the gateway has no live tool window', async () => {
        const requestGateway = vi.fn(async () => ({ status: 'rejected' }));
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), refreshSessions: async () => undefined, requestGateway: requestGateway }));
...
describe('usePromptActions restoreToMessage', () => {
    ...
    it('rewinds to the target user turn and resubmits its text', async () => {
        ...
        let lastState = {};
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), onSeedState: state => (lastState = state), refreshSessions: async () => undefined, requestGateway: requestGateway, seedMessages: $messages.get() }));
        ...
        expect(lastState.messages.map(m => m.id)).toEqual(['u1']);
    ...
    it('rethrows gateway failures and clears the busy flags for the dialog to surface', async () => {
        ...
        let lastState = {};
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), onSeedState: state => (lastState = state), refreshSessions: async () => undefined, requestGateway: requestGateway }));
    ...
    it('interrupts the live turn and retries past "session busy" when reverting mid-stream', async () => {
        ...
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), refreshSessions: async () => undefined, requestGateway: requestGateway, seedMessages: $messages.get() }));
        await handle.restoreToMessage('u1');
...
describe('usePromptActions file attachment sync', () => {
    ...
    it('passes the path directly via file.attach in local mode (no byte upload)', async () => {
        ...
        const requestGateway = vi.fn(async (method, params) => {
            calls.push({ method, params });
            if (method === 'file.attach') {
                return { attached: true, ref_text: '@file:data/report.txt', uploaded: false };
            }
            return {};
        });
        let handle = null;
    ...
    it('joins an in-flight eager upload at submit instead of staging the file twice', async () => {
        ...
        const requestGateway = vi.fn(async (method) => {
            ...
            if (method === 'file.attach') {
                ...
                return { attached: true, ref_text: '@file:.hermes/desktop-attachments/doc.pdf', uploaded: true };
            }
            return {};
        });
        let handle = null;
...
describe('usePromptActions sleep/wake session recovery', () => {
    ...
    it('resumes the stored session and retries once when prompt.submit reports "session not found"', async () => {
        ...
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), refreshSessions: async () => undefined, requestGateway: requestGateway, storedSessionId: STORED_SESSION_ID }));
    ...
    it('surfaces the original error (no resume) when the failure is not "session not found"', async () => {
        ...
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), onSeedState: s => states.push(s), refreshSessions: async () => undefined, requestGateway: requestGateway, storedSessionId: STORED_SESSION_ID }));
    ...
    it('resumes the SELECTED stored session instead of minting a new one when activeSessionId is null (#55578 split)', async () => {
        ...
        let handle = null;
        render(_jsx(Harness, { activeSessionId: null, createBackendSessionForSend: createBackendSessionForSend, onReady: h => (handle = h), refreshSessions: async () => undefined, requestGateway: requestGateway, storedSessionId: STORED_SESSION_ID }));
    ...
    it('still creates a new session for a genuine new-chat draft (no stored session selected)', async () => {
        ...
        let handle = null;
        render(_jsx(Harness, { activeSessionId: null, createBackendSessionForSend: createBackendSessionForSend, onReady: h => (handle = h), refreshSessions: async () => undefined, requestGateway: requestGateway, storedSessionId: null }));
...
describe('usePromptActions submit session-context isolation (#54527)', () => {
    ...
    it('aborts submit when the user switches sessions during session.resume (no misroute)', async () => {
        ...
        let releaseResume = () => { };
        const calls = [];
        const selectedStoredSessionIdRef = { current: STORED_SESSION_A };
        ...
        const requestGateway = vi.fn(async (method, params) => {
            ...
            if (method === 'session.resume') {
                ...
                return { session_id: RUNTIME_SESSION_B };
            }
            return {};
        });
        let handle = null;
        render(_jsx(Harness, { activeSessionId: null, activeSessionIdRef: activeSessionIdRef, onReady: h => (handle = h), refreshSessions: async () => undefined, requestGateway: requestGateway, selectedStoredSessionIdRef: selectedStoredSessionIdRef, storedSessionId: STORED_SESSION_A }));
        await waitFor(() => expect(handle).not.toBeNull());
    ...
    it('aborts recovery submit when the user switches sessions during timeout resume', async () => {
        ...
        let handle = null;
        render(_jsx(Harness, { onReady: h => (handle = h), refreshSessions: async () => undefined, requestGateway: requestGateway, selectedStoredSessionIdRef: selectedStoredSessionIdRef, storedSessionId: STORED_SESSION_A }));
        await waitFor(() => expect(handle).not.toBeNull());
...
describe('usePromptActions eager attachment upload (drop-time)', () => {
    ...
    it('uploads a dropped file the moment it lands (active session) and rewrites the chip with the gateway ref', async () => {
        ...
        render(_jsx(Harness, { onReady: () => undefined, refreshSessions: async () => undefined, requestGateway: requestGateway }));
...
describe('uploadComposerAttachment remote read failures', () => {
    ...
    it('turns the raw 16MB IPC cap error into a friendly remote-gateway message', async () => {
        ...
        const requestGateway = vi.fn(async () => ({}));
        await expect(uploadComposerAttachment({ id: 'file:big', kind: 'file', label: 'huge.csv', path: '/abs/huge.csv' }, { remote: true, requestGateway, sessionId: RUNTIME_SESSION_ID })).rejects.toThrow('huge.csv is too large to upload to the remote gateway (max 16 MB).');
    ...
    it('passes non-cap read errors through unchanged', async () => {
        ...
        await expect(uploadComposerAttachment({ id: 'file:gone', kind: 'file', label: 'gone.csv', path: '/abs/gone.csv' }, { remote: true, requestGateway: vi.fn(async () => ({})), sessionId: RUNTIME_SESSION_ID })).rejects.toThrow('ENOENT: no such file');
    });
});
