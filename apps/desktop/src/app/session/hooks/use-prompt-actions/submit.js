...
export function useSubmitPrompt(deps) {
    const { activeSessionId, activeSessionIdRef, busyRef, copy, createBackendSessionForSend, getRouteToken, requestGateway, selectedStoredSessionIdRef, syncAttachmentsForSubmit, updateSessionState, scope = MAIN_SUBMIT_SCOPE } = deps;
    return useCallback(async (rawText, options) => {
        ...
        const attachments = (options?.attachments ?? scope.readAttachments()).filter((a) => Boolean(a));
