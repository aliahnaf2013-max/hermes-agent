...
import { $busy, $messages, noteSessionActivity, onSessionWatchdogClear, setCurrentFastMode, setCurrentModel, setCurrentPersonality, setCurrentProvider, setCurrentReasoningEffort, setCurrentServiceTier, setSessionAttention, setSessionWorking, setTurnStartedAt, setYoloActive } from '@/store/session';
...
function syncRuntimeMetadataToView(state) {
...
export function useSessionStateCache({ activeSessionId, busyRef, selectedStoredSessionId, setAwaitingResponse, setBusy, setMessages }) {
    const busy = useStore($busy);
    const activeSessionIdRef = useRef(null);
    const selectedStoredSessionIdRef = useRef(null);
    ...
    const ensureSessionState = useCallback((sessionId, storedSessionId) => {
    ...
    const flushPendingViewState = useCallback(() => {
        ...
        const nextMessages = viewSessionIdRef.current === pending.sessionId
    ...
    const syncSessionStateToView = useCallback((sessionId, state) => {
    ...
    const updateSessionState = useCallback((sessionId, updater, storedSessionId) => {
