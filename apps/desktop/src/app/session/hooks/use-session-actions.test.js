...
import { $activeSessionId, $currentCwd, $messages, $resumeFailedSessionId, setActiveSessionId, setMessages, setResumeFailedSessionId, setSessions } from '@/store/session';
...
function Harness({ onReady, requestGateway }) {
...
async function createWith(profileSetup) {
    ...
    const requestGateway = vi.fn(async (method, params) => {
        if (method === 'session.create') {
            createParams = params;
            return { session_id: RUNTIME_SESSION_ID, stored_session_id: null };
        }
        return {};
    });
    $currentCwd.set('');
    profileSetup();
    let create = null;
    render(_jsx(Harness, { onReady: c => (create = c), requestGateway: requestGateway }));
...
function ResumeHarness({ onReady, requestGateway, runtimeIdByStoredSessionIdRef, sessionStateByRuntimeIdRef }) {
    const ref = (value) => ({ current: value });
    const actions = useSessionActions({
        activeSessionId: null,
        activeSessionIdRef: ref(null),
        ...
        navigate: vi.fn(),
        requestGateway,
        runtimeIdByStoredSessionIdRef: runtimeIdByStoredSessionIdRef ?? ref(new Map()),
        selectedStoredSessionId: null,
        selectedStoredSessionIdRef: ref(null),
        sessionStateByRuntimeIdRef: sessionStateByRuntimeIdRef ?? ref(new Map()),
        syncSessionStateToView: vi.fn(),
        updateSessionState: (_sessionId, updater) => updater({})
    });
    useEffect(() => {
...
describe('resumeSession failure recovery', () => {
    ...
    async function runResume(requestGateway, options = {}) {
        let resume = null;
        render(_jsx(ResumeHarness, { onReady: r => (resume = r), requestGateway: requestGateway, ...options }));
    ...
    it('leaves the failure latch clear when resume succeeds', async () => {
        ...
        const requestGateway = vi.fn(async (method, params) => {
            if (method === 'session.resume') {
                return { session_id: 'runtime-1', resumed: params?.session_id, messages: [], info: {} };
            }
            return {};
        });
    ...
    it('arms the failure latch when resume succeeds with an empty transcript for a non-empty stored session', async () => {
        ...
        vi.mocked(getSessionMessages).mockResolvedValue({ messages: [], session_id: 'stored-1' });
        await runResume(requestGateway);
        expect($resumeFailedSessionId.get()).toBe('stored-1');
...
function BranchHarness({ onReady, requestGateway }) {
    const ref = (value) => ({ current: value });
    const actions = useSessionActions({
        activeSessionId: null,
        activeSessionIdRef: ref(null),
...
describe('branchStoredSession desktop source tagging', () => {
    ...
    it('tags desktop branch sessions as desktop sessions', async () => {
        ...
        const requestGateway = vi.fn(async (method, params) => {
            if (method === 'session.create') {
                createParams = params;
                return { session_id: 'branch-runtime', stored_session_id: 'branch-stored' };
            }
            return {};
        });
        ...
        let branchStoredSession = null;
        render(_jsx(BranchHarness, { onReady: branch => (branchStoredSession = branch), requestGateway: requestGateway }));
...
const clientState = (storedSessionId) => createClientSessionState(storedSessionId);
describe('resumeSession warm-cache mapping integrity', () => {
    ...
    it('rejects a cross-wired runtime mapping and falls through to a full resume', async () => {
        ...
        const requestGateway = vi.fn(async (method, params) => {
            if (method === 'session.resume') {
                return { session_id: 'rt-A-fresh', resumed: params?.session_id, messages: [], info: {} };
            }
            return {};
        });
        vi.mocked(getSessionMessages).mockResolvedValue({ messages: [] });
        let resume = null;
        render(_jsx(ResumeHarness, { onReady: r => (resume = r), requestGateway: requestGateway, runtimeIdByStoredSessionIdRef: runtimeIdByStoredSessionIdRef, sessionStateByRuntimeIdRef: sessionStateByRuntimeIdRef }));
        await waitFor(() => expect(resume).not.toBeNull());
