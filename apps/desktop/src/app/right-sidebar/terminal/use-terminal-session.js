...
import { isAddSelectionShortcut, resolveSurfaceColor, terminalSelectionAnchor, terminalSelectionLabel, terminalTheme } from './selection';
...
function previewSelectionLabel() {
...
function readEscapeSequence(data, index) {
...
function keepEscapeSequences(data) {
    let index = 0;
    let out = '';
    while (index < data.length) {
...
function stripInitialPromptGap(data) {
    let index = 0;
    let prefix = '';
    while (index < data.length) {
        const sequence = readEscapeSequence(data, index);
...
const visibleText = (line) => stripEscapeSequences(line).replace(/[\s%]/g, '');
...
export function cleanReviveSnapshot(serialized) {
...
export function isIdlePromptOnly(serialized) {
...
export function parseOscCwd(code, payload) {
    if (code === 9) {
...
function collectDroppedPaths(t) {
    const seen = new Set();
    const push = (value) => {
...
function quotePathForShell(path, shellName) {
...
export function useTerminalSession({ id, cwd, active, onAddSelectionToChat, restoreCwd, reviveBuffer, onShell }) {
    ...
    const lastObservedCwdRef = useRef(null);
    ...
    const readSelection = useCallback(() => termRef.current?.getSelection() || window.getSelection()?.toString() || '', []);
    ...
    useEffect(() => {
        ...
        const cwdOscHandlers = [7, 9].map(code => term.parser.registerOscHandler(code, payload => {
        ...
        const startSession = () => void terminalApi
            ...
            setStatus('open');
            cleanup.push(terminalApi.onData(session.id, data => {
                armedWrite(data);
                scheduleSnapshot();
