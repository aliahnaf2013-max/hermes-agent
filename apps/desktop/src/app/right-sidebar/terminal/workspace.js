...
export function TerminalWorkspace({ onAddSelectionToChat }) {
    ...
    return (_jsx(_Fragment, { children: terminals.map(term => term.kind === 'agent' ? (_jsx(AgentTerminalInstance, { active: term.id === activeId, id: term.id, procId: term.procId }, term.id)) : (_jsx(TerminalInstance, { active: term.id === activeId, cwd: term.cwd, id: term.id, onAddSelectionToChat: onAddSelectionToChat, restoreCwd: term.restoreCwd, reviveBuffer: term.reviveBuffer }, term.id))) }));
}
