...
import { DropdownMenuGroup, DropdownMenuItem, DropdownMenuLabel, dropdownMenuRow, DropdownMenuSearch, dropdownMenuSectionLabel, DropdownMenuSeparator, DropdownMenuSub, DropdownMenuSubTrigger } from '@/components/ui/dropdown-menu';
...
import { $visibleModels, collapseModelFamilies, DEFAULT_VISIBLE_PER_PROVIDER, effectiveVisibleKeys, modelVisibilityKey, setModelVisibilityOpen } from '@/store/model-visibility';
import { $activeSessionId, $currentFastMode, $currentModel, $currentProvider, $currentReasoningEffort } from '@/store/session';
...
export function ModelMenuPanel({ gateway, onSelectModel, requestGateway }) {
    const { t } = useI18n();
    ...
    const { model: optionsModel, provider: optionsProvider } = currentPickerSelection(!!activeSessionId, { model: currentModel, provider: currentProvider }, modelOptions.data);
    ...
    const moaPresets = useMemo(() => providers?.find(provider => provider.slug.toLowerCase() === 'moa')?.models ?? [], [providers]);
    const pickerProviders = useMemo(() => providers?.filter(provider => provider.slug.toLowerCase() !== 'moa') ?? [], [providers]);
    const effectiveVisibleModels = useMemo(() => effectiveVisibleKeys(visibleModels, pickerProviders), [visibleModels, pickerProviders]);
    ...
    const switchTo = (model, provider) => onSelectModel({ model, provider });
    ...
    const selectMoaPreset = async (preset) => {
        if ((await switchTo(preset, 'moa')) === false) {
            return;
        }
        closeMenu();
    };
    const groups = useMemo(() => groupModels(pickerProviders, search, { model: optionsModel, provider: optionsProvider }, effectiveVisibleModels), [pickerProviders, search, optionsModel, optionsProvider, effectiveVisibleModels]);
    return (_jsxs(_Fragment, { children: [_jsx(DropdownMenuSearch, { "aria-label": copy.search, onValueChange: setSearch, placeholder: copy.search, value: search }), _jsx(DropdownMenuSeparator, { className: "mx-0" }), loading ? (_jsx(DropdownMenuGroup, { className: "py-1", children: Array.from({ length: 4 }, (_, index) => (_jsx(DropdownMenuItem, { className: dropdownMenuRow, disabled: true, onSelect: event => event.preventDefault(), children: _jsx(Skeleton, { className: "h-4 w-full" }) }, index))) })) : error ? (_jsx(DropdownMenuItem, { className: dropdownMenuRow, disabled: true, children: error })) : groups.length === 0 && moaPresets.length === 0 ? (_jsx(DropdownMenuItem, { className: dropdownMenuRow, disabled: true, children: copy.noModels })) : (_jsx("div", { className: "max-h-[max(150px,30dvh)] overflow-y-auto py-0.5", children: groups.map(group => (_jsxs(DropdownMenuGroup, { className: "py-0.5", children: [_jsx(DropdownMenuLabel, { className: dropdownMenuSectionLabel, children: group.provider.name }), group.families.map(family => {
                            // The active id may be the base or its -fast sibling; either
                            // way this one family row represents both.
                            const activeId = group.provider.slug === optionsProvider &&
                            ...
                            const fastControl = resolveFastControl(activeId ?? family.id, group.provider.models ?? [], caps?.fast ?? false, effFast);
                            const meta = [
                            ...
                            return (_jsxs(DropdownMenuSub, { children: [_jsxs(DropdownMenuSubTrigger, { className: dropdownMenuRow, hideChevron: true, onClick: activate, onKeyDown: event => {
                                            if (event.key === 'Enter' || event.key === ' ') {
                                                activate();
                                            }
                                        }, children: [_jsxs("span", { className: "min-w-0 flex-1 truncate", children: [name, meta ? _jsxs("span", { className: "text-(--ui-text-tertiary)", children: [" ", meta] }) : null] }), isCurrent ? _jsx(Codicon, { className: "ml-auto text-foreground", name: "check", size: "0.75rem" }) : null] }), _jsx(ModelEditSubmenu, { effort: effEffort, fastControl: fastControl, isActive: isCurrent, model: family.id, onSelectModel: nextModel => switchTo(nextModel, group.provider.slug), provider: group.provider.slug, reasoning: caps?.reasoning ?? true, requestGateway: requestGateway })] }, `${group.provider.slug}:${family.id}`));
                        })] }, group.provider.slug))) })), _jsx(DropdownMenuSeparator, { className: "mx-0" }), moaPresets.length > 0 ? (_jsxs(_Fragment, { children: [_jsx(DropdownMenuLabel, { className: dropdownMenuSectionLabel, children: "MoA presets" }), moaPresets.map(preset => {
                        const isCurrentMoa = optionsProvider === 'moa' && optionsModel === preset;
                        return (_jsxs(DropdownMenuItem, { className: dropdownMenuRow, onSelect: event => {
                                event.preventDefault();
                                void selectMoaPreset(preset);
                            }, children: [_jsxs("span", { className: "min-w-0 flex-1 truncate", children: ["MoA: ", preset] }), isCurrentMoa ? _jsx(Codicon, { className: "ml-auto text-foreground", name: "check", size: "0.75rem" }) : null] }, `moa:${preset}`));
                    }), _jsx(DropdownMenuSeparator, { className: "mx-0" })] })) : null, _jsxs(DropdownMenuItem, { className: cn(dropdownMenuRow, 'text-(--ui-text-tertiary)'), disabled: refreshing, onSelect: event => {
                    event.preventDefault();
                    void refreshModels();
                }, children: [_jsx(Codicon, { className: cn(refreshing && 'animate-spin'), name: "sync", size: "0.75rem" }), copy.refreshModels] }), _jsxs(DropdownMenuItem, { className: cn(dropdownMenuRow, 'text-(--ui-text-tertiary)'), onSelect: () => setModelVisibilityOpen(true), children: [_jsx(Codicon, { name: "settings-gear", size: "0.75rem" }), copy.editModels] })] }));
}
...
function groupModels(providers, search, current, visible) {
    ...
    for (const provider of providers) {
        ...
        const matches = (family) => `${family.id} ${family.fastId ?? ''} ${provider.name} ${provider.slug} ${displayModelName(family.id)}`
            .toLowerCase()
            .includes(q);
        ...
        if (q) {
            ...
            shown = new Set(allFamilies.filter(family => visible.has(modelVisibilityKey(provider.slug, family.id))).map(family => family.id));
        }
        else {
        ...
        const activeId = provider.slug === current.provider && current.model
