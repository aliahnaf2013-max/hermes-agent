...
import { requestSessionSearchFocus, setFileBrowserOpen, toggleFileBrowserOpen, togglePanesFlipped, toggleSidebarOpen } from '@/store/layout';
import { $newChatProfile, cycleProfile, requestProfileCreate, switchProfileToSlot, switchToDefaultProfile, toggleShowAllProfiles } from '@/store/profile';
import { requestNewWorktree } from '@/store/projects';
import { toggleReview } from '@/store/review';
import { setModelPickerOpen } from '@/store/session';
import { $switcherOpen, closeSwitcher, commitOnCtrlUp, onSwitcherTabDown, onSwitcherTabUp, openOrAdvanceSwitcher, slotSessionId, switcherActive, switcherJustClosed } from '@/store/session-switcher';
...
import { AGENTS_ROUTE, ARTIFACTS_ROUTE, CRON_ROUTE, MESSAGING_ROUTE, PROFILES_ROUTE, sessionRoute, SETTINGS_ROUTE, SKILLS_ROUTE } from '../routes';
...
export function useKeybinds(deps) {
    ...
    const commitSwitcherRef = useRef(() => { });
    const profileSwitchHandlers = {};
    ...
    handlersRef.current = {
        ...
        'view.toggleRightSidebar': () => layoutHasRootSide('right') ? toggleFileBrowserOpen() : setTerminalTakeover(!$terminalTakeover.get()),
