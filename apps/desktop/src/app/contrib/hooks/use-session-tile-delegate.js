 ...
 */
export function useSessionTileDelegate({ executeSlashCommand, requestGateway, runtimeIdByStoredSessionIdRef, sessionStateByRuntimeIdRef, updateSessionState }) {
    useEffect(() => {
        setSessionTileDelegate({
            ...
            resumeTile: async (storedSessionId) => {
                ...
                const [prefetch, resumed] = await Promise.all([
                    ...
                    requestGateway('session.resume', { session_id: storedSessionId, cols: 96 })
                ]);
