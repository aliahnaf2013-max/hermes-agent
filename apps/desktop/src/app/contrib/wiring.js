...
import { $activeSessionId, $connection, $currentCwd, $freshDraftReady, $gatewayState, $messages, $messagingSessions, $resumeExhaustedSessionId, $resumeFailedSessionId, $selectedStoredSessionId, $sessions, sessionMatchesStoredId, sessionPinId, setAwaitingResponse, setBusy, setCurrentBranch, setCurrentCwd, setCurrentModel, setCurrentProvider, setMessages } from '@/store/session';
...
export function ContribWiring({ children }) {
    ...
    const { activeSessionIdRef, ensureSessionState, runtimeIdByStoredSessionIdRef, selectedStoredSessionIdRef, sessionStateByRuntimeIdRef, syncSessionStateToView, updateSessionState } = useSessionStateCache({
        activeSessionId,
        busyRef,
    ...
    const { loadMoreMessagingForPlatform, loadMoreSessions, loadMoreSessionsForProfile, refreshCronJobs, refreshMessagingSessions, refreshSessions } = useSessionListActions({ profileScope });
    const updateActiveSessionRuntimeInfo = useCallback((info) => {
    ...
    const hydrateFromStoredSession = useCallback(async (attempts = 1, storedSessionId = selectedStoredSessionIdRef.current, runtimeSessionId = activeSessionIdRef.current) => {
        ...
        for (let index = 0; index < Math.max(1, attempts); index += 1) {
            try {
                ...
                updateSessionState(runtimeSessionId, state => ({ ...state, messages: preserveLocalAssistantErrors(messages, state.messages) }), storedSessionId);
    ...
    const { archiveSession, branchCurrentSession, branchStoredSession, createBackendSessionForSend, openNewSessionTile, removeSession, resumeSession, selectSidebarItem, startFreshSessionDraft } = useSessionActions({
        activeSessionId,
        activeSessionIdRef,
        busyRef,
        creatingSessionRef,
    ...
    const startSessionInWorkspace = useCallback((path) => {
        startFreshSessionDraft();
    ...
    const branchInNewChat = useCallback(async (messageId) => {
    ...
    const { cancelRun, editMessage, executeSlashCommand, handleThreadMessagesChange, reloadFromMessage, restoreToMessage, steerPrompt, submitText, transcribeVoiceAudio } = usePromptActions({
        activeSessionId,
        activeSessionIdRef,
    ...
    const dismissError = useCallback((messageId) => {
        ...
        const clearErrorIn = (messages) => messages.flatMap(message => {
    ...
    const handleGatewayEventWithPlugins = useCallback((event) => {
    ...
    const activeIsMessaging = !!selectedStoredSessionId &&
    ...
    const sidebarNode = useMemo(() => _jsx(SidebarSurface, { actions: actions, currentView: currentView }), [actions, currentView]);
    const terminalNode = useMemo(() => _jsx(TerminalSurface, {}), []);
    const statusbarNode = useMemo(() => (_jsx(StatusbarSurface, { actions: actions, agentsOpen: agentsOpen, chatOpen: chatOpen, commandCenterOpen: commandCenterOpen })), [actions, agentsOpen, chatOpen, commandCenterOpen]);
    // Chat surface only — full-page views (skills/messaging/…) own their bars.
    const sessionTitleNode = useMemo(() => chatOpen && !isSecondaryWindow() ? (_jsx(SessionTitleSurface, { actions: actions, isRoutedSessionView: Boolean(routedSessionId) })) : null, [actions, chatOpen, routedSessionId]);
    ...
    const chatRoutesNode = useMemo(() => _jsx(ChatRoutesSurface, { actions: actions, maxVoiceRecordingSeconds: voiceMaxRecordingSeconds }), [actions, voiceMaxRecordingSeconds]);
    const api = useMemo(() => ({
    ...
    const titlebarToolsWidth = paneToolCount > 0
        ...
        : systemToolsWidth;
    return (_jsxs(ContribWiringContext.Provider, { value: api, children: [_jsxs("div", { className: "contents", style: {
                ...
                }, children: [_jsx(TitlebarControls, { leftTools: leftTitlebarTools, onOpenSettings: () => navigate(SETTINGS_ROUTE), tools: rightTitlebarTools }), children] }), _jsx(RemoteDisplayBanner, {}), !isSecondaryWindow() && _jsx(DesktopInstallOverlay, {}), !isSecondaryWindow() && (_jsx(DesktopOnboardingOverlay, { enabled: gatewayState === 'open', onCompleted: () => {
                    void refreshHermesConfig();
                    void refreshCurrentModel();
                    void queryClient.invalidateQueries({ queryKey: ['model-options'] });
                }, requestGateway: requestGateway })), _jsx(ModelPickerOverlay, { gateway: gatewayRef.current || undefined, onSelect: selectModel }), _jsx(SessionPickerOverlay, { onResume: resumeSession }), _jsx(ModelVisibilityOverlay, { gateway: gatewayRef.current || undefined, onOpenProviders: openProviderSettings }), _jsx(UpdatesOverlay, {}), _jsx(GatewayConnectingOverlay, {}), _jsx(BootFailureOverlay, {}), _jsx(CommandPalette, {}), _jsx(PetGenerateOverlay, {}), _jsx(SessionSwitcher, {}), _jsx(FileActionDialogs, {}), _jsx(RemoteFolderPicker, {}), settingsOpen && (_jsx(Suspense, { fallback: null, children: _jsx(SettingsView, { gateway: gatewayRef.current, onClose: closeOverlayToPreviousRoute, onConfigSaved: () => {
                        void refreshHermesConfig();
                        void refreshCurrentModel();
                        void queryClient.invalidateQueries({ queryKey: ['model-options'] });
                        ...
                        void refreshCurrentModel();
                        void queryClient.invalidateQueries({ queryKey: ['model-options'] });
                    } }) })), commandCenterOpen && (_jsx(Suspense, { fallback: null, children: _jsx(CommandCenterView, { initialSection: commandCenterInitialSection, onClose: closeOverlayToPreviousRoute, onDeleteSession: removeSession, onNavigateRoute: path => navigate(path), onOpenSession: sessionId => navigate(sessionRoute(sessionId)) }) })), agentsOpen && (_jsx(Suspense, { fallback: null, children: _jsx(AgentsView, { onClose: closeOverlayToPreviousRoute }) })), cronOpen && (_jsx(Suspense, { fallback: null, children: _jsx(CronView, { onClose: closeOverlayToPreviousRoute, onOpenSession: sessionId => navigate(sessionRoute(sessionId)) }) })), profilesOpen && (_jsx(Suspense, { fallback: null, children: _jsx(ProfilesView, { onClose: closeOverlayToPreviousRoute }) })), starmapOpen && (_jsx(Suspense, { fallback: null, children: _jsx(StarmapView, { onClose: closeOverlayToPreviousRoute }) })), _jsx(KeybindPanel, {}), _jsx(NotificationStack, {}), _jsx(FloatingPet, {}), _jsx(PersistentTerminal, { onAddSelectionToChat: composer.addTerminalSelectionAttachment })] }));
}
