...
import { $layoutTree, bindTreeSideVisibility, declareDefaultTree, dockPaneBeside, mirrorLayoutTree, registerLayoutResetHandler, registerPaneCloser, registerPaneOpener, resetLayoutTree, revealTreePane, setTreePaneHidden, watchContributedPanes } from '@/components/pane-shell/tree/store';
...
import { $fileBrowserOpen, $panesFlipped, $sidebarOpen, FILE_BROWSER_DEFAULT_WIDTH, FILE_BROWSER_MAX_WIDTH, FILE_BROWSER_MIN_WIDTH, setFileBrowserOpen, setSidebarOpen, SIDEBAR_DEFAULT_WIDTH, SIDEBAR_MAX_WIDTH } from '@/store/layout';
...
registry.registerMany([
    {
        ...
        render: () => _jsx(WiredPane, { part: "sidebar" })
    },
    {
        id: 'workspace',
        area: 'panes',
        ...
        render: () => _jsx(WiredPane, { part: "chatRoutes" })
    },
    {
        id: 'terminal',
        area: 'panes',
        ...
        render: () => _jsx(WiredPane, { part: "terminal" })
    },
    {
        id: 'files',
        area: 'panes',
...
registry.registerMany([
    ...
    { id: 'session-title', area: 'titleBar.center', order: 0, render: () => _jsx(WiredPane, { part: "sessionTitle" }) },
...
const FOCUS_TREE = split('row', [group(['sessions']), group(['workspace', 'files', 'preview', 'review', 'terminal'])], [1, 4.6]);
const TERMINAL_TREE = split('column', [
    split('row', [group(['sessions']), group(['workspace']), group(['files', 'preview', 'review'])], [1, 3.2, 1.2]),
    group(['terminal'])
], [3, 1]);
const QUAD_TREE = split('column', [
...
// ---------------------------------------------------------------------------
function bindPaneVisibility(paneId, $open, close, open) {
...
bindPaneVisibility('review', computed([$reviewOpen, $hasWorkspace], (open, workspace) => open && workspace), closeReview);
...
const $previewVisible = computed([$previewTarget, $filePreviewTarget], (target, fileTarget) => Boolean(target || fileTarget));
...
export function ContribController() {
    ...
    return (_jsx(SidebarProvider, { className: "h-screen min-h-0 flex-col bg-background", onOpenChange: setSidebarOpen, open: sidebarOpen, style: { '--sidebar-width': '100%' }, children: _jsx(ContribWiring, { children: _jsxs("div", { className: "flex h-screen min-h-0 w-screen flex-col bg-(--ui-bg-chrome) text-(--ui-text-primary)", style: { '--titlebar-height': '0px' }, children: [_jsxs("div", { className: "relative flex h-[34px] shrink-0 items-center border-b border-(--ui-stroke-tertiary) text-xs", style: {
                        ...
                        }, children: [_jsx("div", { "aria-hidden": "true", className: "pointer-events-none absolute inset-y-0 left-0 w-(--titlebar-controls-left,14px) [-webkit-app-region:drag]" }), _jsx("div", { "aria-hidden": "true", className: "pointer-events-none absolute inset-y-0 left-[calc(var(--titlebar-controls-left,14px)+(var(--titlebar-control-size,1.25rem)*2)+0.75rem)] right-[calc(var(--titlebar-tools-right,0.75rem)+var(--titlebar-tools-width,5.5rem)+0.75rem)] [-webkit-app-region:drag]" }), _jsx("div", { className: "pointer-events-auto relative z-10 flex w-max items-center gap-2 [-webkit-app-region:no-drag]", children: _jsx(Slot, { area: "titleBar.left" }) }), _jsx("div", { className: "pointer-events-auto absolute left-1/2 top-1/2 z-10 flex w-max -translate-x-1/2 -translate-y-1/2 items-center gap-2 [-webkit-app-region:no-drag]", children: _jsx(Slot, { area: "titleBar.center" }) }), _jsx("div", { className: "pointer-events-auto relative z-10 ml-auto flex w-max items-center gap-2 [-webkit-app-region:no-drag]", children: _jsx(Slot, { area: "titleBar.right" }) })] }), _jsx(LayoutTreeRoot, {}), _jsx(WiredPane, { part: "statusbar" })] }) }) }));
}
