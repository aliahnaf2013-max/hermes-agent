...
const isConnectingShown = () => screen.queryAllByText((_, el) => /^CONN[/\\|\-_=+<>~:*A-Z]*$/.test(el?.textContent?.trim() ?? '')).length > 0;
const isRecoveryShown = () => Boolean(screen.queryByText(/use local gateway/i) || screen.queryByText(/retry/i) || screen.queryByText(/sign in/i));
describe('connecting overlay vs recovery surface', () => {
    it('hard initial-boot failure surfaces the recovery overlay (the working path)', () => {
        ...
        render(_jsxs(_Fragment, { children: [_jsx(GatewayConnectingOverlay, {}), _jsx(BootFailureOverlay, {})] }));
    ...
    it('post-boot socket drops do not re-cover the app with the initial CONNECTING overlay', () => {
        ...
        const { rerender } = render(_jsxs(_Fragment, { children: [_jsx(GatewayConnectingOverlay, {}), _jsx(BootFailureOverlay, {})] }));
        expect(isConnectingShown()).toBe(false);
        ...
        rerender(_jsxs(_Fragment, { children: [_jsx(GatewayConnectingOverlay, {}), _jsx(BootFailureOverlay, {})] }));
