 ...
 */
function assignZones(zones, panes) {
    const geo = zones.map(z => ({
        index: z.index,
    ...
    const rank = (p) => (p.placement === 'main' ? 0 : p.placement ? 1 : 2);
 ...
 */
export function gridToTree(gridModel, panes) {
...
export function gridIsTreeExpressible(gridModel) {
    const zones = modelToZones(gridModel);
    if (!zones) {
        return false;
    }
