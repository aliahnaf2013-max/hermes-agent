...
import { canSplit, doMerge, dragResizer, initColumns, initGrid, initPriorityGrid, initRows, mergeClosureIndices, modelToResizers, modelToZones, MULTIPLIER, splitZone } from './grid-model';
...
const pct = (v) => `${(v / MULTIPLIER) * 100}%`;
/** Resizer screen geometry derived from its zones (model units). */
function resizerGeometry(resizer, zones) {
...
export function ZoneEditor() {
    ...
    const [model, setModel] = useState(() => initPriorityGrid(3));
    ...
    const toModelPoint = useCallback((clientX, clientY) => {
    ...
    const zoneAt = (x, y) => zones.find(z => x >= z.left && x < z.right && y >= z.top && y < z.bottom) ?? null;
    const updateSplitPreview = (clientX, clientY) => {
        ...
        setSplitPreview(canSplit(model, zone.index, position, orientation) ? { zoneIndex: zone.index, orientation, position } : null);
    };
    const onCanvasPointerDown = (e) => {
        if (e.button !== 0 || e.target.dataset.resizer !== undefined) {
            return;
        }
        e.preventDefault();
        setMergeAt(null);
    ...
    const startResizerDrag = (index, e) => {
        e.preventDefault();
        e.stopPropagation();
        const rect = canvasRef.current.getBoundingClientRect();
    ...
    const save = () => {
        ...
        const placed = paneIds.map(id => ({
            id,
            placement: contributions.find(c => c.id === id)?.data
                ?.placement
        }));
    ...
    return (_jsxs("div", { className: "absolute inset-0 z-[70] flex flex-col gap-3 p-6 [-webkit-app-region:no-drag]", style: { background: 'color-mix(in srgb, var(--ui-bg-chrome) 88%, transparent)', backdropFilter: 'blur(6px)' }, children: [_jsxs("div", { className: "flex items-end justify-between gap-3", children: [_jsxs("div", { className: "min-w-0", children: [_jsx("h2", { className: "text-sm font-semibold text-foreground", children: t.zones.zoneEditorTitle }), _jsxs("p", { className: "text-xs text-muted-foreground/80", children: [t.zones.editorHintPre, _jsx("kbd", { className: "rounded border border-(--ui-stroke-secondary) bg-foreground/5 px-1 font-mono text-[10px]", children: "\u21E7" }), t.zones.editorHintPost] })] }), _jsxs("div", { className: "flex shrink-0 items-center gap-1.5", children: [templates.map(t => (_jsx(Button, { onClick: () => {
                                    setSelection([]);
                                    setMergeAt(null);
                                    setModel(t.make(templateCount));
                                }, size: "sm", variant: "outline", children: t.label }, t.label))), _jsx(Input, { className: "h-7 w-14 text-center text-xs", max: 8, min: 1, onChange: e => setTemplateCount(Math.max(1, Math.min(8, Number(e.target.value) || 1))), type: "number", value: templateCount })] })] }), _jsxs("div", { className: "relative min-h-0 flex-1 cursor-crosshair overflow-hidden rounded-lg border border-(--ui-stroke-secondary)", onPointerDown: onCanvasPointerDown, onPointerLeave: () => setSplitPreview(null), onPointerMove: e => updateSplitPreview(e.clientX, e.clientY), ref: canvasRef, children: [zones.map(zone => {
                        const selected = selection.includes(zone.index);
                        return (_jsx("div", { className: "absolute flex items-center justify-center rounded-[3px] border transition-colors", style: {
                                left: pct(zone.left),
                                top: pct(zone.top),
                            ...
                            }, children: _jsx("span", { className: "select-none text-[0.64rem] font-semibold uppercase tracking-[0.16em] text-(--ui-text-tertiary)", children: t.zones.zoneTag(zone.index + 1) }) }, zone.index));
                    }), splitPreview &&
                        (() => {
                            ...
                            return (_jsx("div", { className: "pointer-events-none absolute", style: {
                        ...
                        })(), selectBox && (_jsx("div", { className: "pointer-events-none absolute border", style: {
                        ...
                        const horizontal = resizer.orientation === 'horizontal';
                        return (_jsx("div", { className: cn('absolute z-10 flex items-center justify-center', horizontal ? 'h-[10px] -translate-y-1/2 cursor-row-resize' : 'w-[10px] -translate-x-1/2 cursor-col-resize'), "data-resizer": i, onPointerDown: e => startResizerDrag(i, e), style: horizontal
                                ? { top: pct(geo.at), left: pct(geo.from), width: pct(geo.to - geo.from) }
                                : { left: pct(geo.at), top: pct(geo.from), height: pct(geo.to - geo.from) }, children: _jsx("span", { className: cn('pointer-events-none rounded-full', horizontal ? 'h-[4px] w-10' : 'h-10 w-[4px]'), style: { background: 'color-mix(in srgb, var(--ui-text-primary) 55%, transparent)' } }) }, `r-${i}`));
                    }), mergeAt && selection.length > 1 && (_jsx("div", { className: "absolute z-20 flex gap-1", style: { left: mergeAt.x, top: mergeAt.y }, children: _jsx(Button, { className: "shadow-lg", onClick: merge, onPointerDown: e => e.stopPropagation(), size: "sm", variant: "outline", children: t.zones.mergeZones(selection.length) }) }))] }), _jsxs("div", { className: "flex items-center gap-1.5", children: [_jsx(Input, { className: "h-7 w-64 text-xs", onChange: e => setName(e.target.value), 
                        ...
                        }, placeholder: t.zones.layoutNamePlaceholder(t.zones.customZoneName(zones.length)), value: name }), _jsx(Button, { disabled: !treeExpressible, onClick: save, size: "sm", variant: "outline", children: t.zones.saveApply }), _jsx(Button, { onClick: () => $zoneEditorOpen.set(false), size: "sm", variant: "ghost", children: t.common.cancel }), !treeExpressible && _jsx("span", { className: "text-xs text-muted-foreground/80", children: t.zones.notExpressible }), _jsx("span", { className: "ml-auto text-xs text-muted-foreground/60", children: t.zones.zoneCount(zones.length) })] })] }));
}
