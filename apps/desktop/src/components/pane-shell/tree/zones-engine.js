...
const zoneArea = (z) => Math.max(0, z.rect.right - z.rect.left) * Math.max(0, z.rect.bottom - z.rect.top);
// ---------------------------------------------------------------------------
// ZonesOverlay::GetAnimationAlpha (verbatim ramp)
// ---------------------------------------------------------------------------
export function getAnimationAlpha(startedAtMs, nowMs, autoHide) {
...
export const FANCYZONES_DEFAULT_COLORS = {
    primaryColor: '#F5FCFF',
    borderColor: '#FFFFFF',
 ...
 */
export function systemThemeZoneColors() {
    return {
...
// ---------------------------------------------------------------------------
function zoneSelectPriority(zones, capturedZones, compare) {
    let chosen = 0;
...
function zoneSelectSubregion(zones, capturedZones, pt, sensitivityRadius) {
    const expand = (rect) => ({
...
function zoneSelectClosestCenter(zones, capturedZones, pt) {
    const getCenter = (zone) => ({
        x: (zone.rect.right + zone.rect.left) / 2,
        y: (zone.rect.top + zone.rect.bottom) / 2
    });
    const pointDifference = (a, b) => (a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y);
    const distanceFromCenter = (zone) => pointDifference(getCenter(zone), pt);
    const closerToCenter = (zone1, zone2) => {
...
// ---------------------------------------------------------------------------
export function zonesFromPoint(zoneList, pt, sensitivityRadius = DEFAULT_SENSITIVITY_RADIUS, overlappingAlgorithm = 'Smallest') {
    const zones = new Map(zoneList.map(z => [z.id, z]));
    const capturedZones = [];
    const strictlyCapturedZones = [];
    for (const zone of zoneList) {
        const zoneRect = zone.rect;
        if (zoneRect.left - sensitivityRadius <= pt.x &&
    ...
    outer: for (let i = 0; i < capturedZones.length; i++) {
        for (let j = i + 1; j < capturedZones.length; j++) {
            ...
            if (Math.max(rectI.top, rectJ.top) + sensitivityRadius < Math.min(rectI.bottom, rectJ.bottom) &&
...
export function primaryZone(zoneList, highlighted, pt) {
...
// ---------------------------------------------------------------------------
export function getCombinedZoneRange(zoneList, initialZones, finalZones) {
    const zones = new Map(zoneList.map(z => [z.id, z]));
...
export class HighlightedZones {
    ...
    update(zoneList, point, selectManyZones) {
        ...
        const updated = highlightZone.length !== this.highlightZone.length || highlightZone.some((z, i) => z !== this.highlightZone[i]);
