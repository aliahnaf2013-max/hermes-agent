...
import { $collapsedTreeSides, $hiddenTreePanes, $narrowViewport, persistTree, presetSplitWeights, setTreeSplitWeights } from '../store';
import { computedPx, cssMax, edgeFixedZone, fixedTrackSize, MIN_PANE_PX, paneChrome, resolveCssPx, rootChildSide, shownPaneIds, subtreeGone } from './track-model';
...
function useSubtreeOverrides(paneIds) {
    ...
    const cache = useRef({ sig: '\0', value: {} });
...
export function TreeSplit({ node, root }) {
    ...
    const paneGone = (id) => !paneFor(id) || (!editMode && hiddenPanes.has(id)) || (narrow && Boolean(paneChrome(paneFor(id)).collapsible));
    const trackCtx = { paneFor, paneGone, overrides };
    ...
    const isEmptyZone = (child) => child.type === 'group' && child.panes.length === 0;
    const isCollapsed = (child) => subtreeGone(child, trackCtx) || (isEmptyZone(child) && !editMode);
    ...
    const sizingFor = (child, track) => {
        ...
        if (shownIds.length <= 1) {
            return paneFor(shownIds[0])?.data ?? null;
        }
        ...
        const all = shownIds.map(id => (paneFor(id)?.data ?? {}));
        const cap = (pick) => all.every(pick) ? cssMax(all.map(pick)) : undefined;
        return {
    ...
    const startSash = useCallback((aIndex, bIndex, e) => {
        ...
        const kidA = container.children[aIndex];
        const kidB = container.children[bIndex];
        if (!kidA || !kidB) {
            return;
        }
    ...
    const resetBoundary = useCallback((aIndex, bIndex) => {
        const container = containerRef.current;
        if (!container) {
            return;
        }
        ...
        for (const i of [aIndex, bIndex]) {
            ...
            for (const paneId of shownPaneIds(child, trackCtx)) {
                const sizing = (paneFor(paneId)?.data ?? {});
    ...
    const isMinimized = (child) => child.type === 'group' && Boolean(child.minimized);
    ...
    const grow = (i) => node.weights[i] / (flexTotal || 1);
    ...
    return (_jsx("div", { className: cn('flex min-h-0 min-w-0 flex-1', horizontal ? 'flex-row' : 'flex-col'), "data-tree-split": node.id, ref: containerRef, children: tracks.map(({ child, collapsed, minimized, narrowCollapsed, sizing, track }, i) => {
            const partner = collapsed ? -1 : seamPartner(i);
            return (_jsxs("div", { className: "relative flex min-h-0 min-w-0", style: collapsed
                    ? { display: 'none' }
                    : minimized
                        ? { flex: '0 0 auto' }
                        : {
                        ...
                        }, children: [partner >= 0 && (_jsx(Sash, { disabled: minimized || tracks[partner].minimized, horizontal: horizontal, onDoubleClick: () => resetBoundary(partner, i), onPointerDown: e => startSash(partner, i, e) })), !narrowCollapsed && (_jsx(TreeNode, { node: child, parentAxis: axis, railSide: horizontal ? railSideFor(i) : undefined }))] }, child.id));
        }) }));
}
function Sash({ disabled, horizontal, onDoubleClick, onPointerDown }) {
    return (_jsxs("div", { className: cn('group absolute z-20 [-webkit-app-region:no-drag]', horizontal ? 'inset-y-0 left-0 w-[9px] -translate-x-1/2' : 'inset-x-0 top-0 h-[9px] -translate-y-1/2', disabled ? 'pointer-events-none' : horizontal ? 'cursor-col-resize' : 'cursor-row-resize'), onDoubleClick: disabled ? undefined : onDoubleClick, onPointerDown: disabled ? undefined : onPointerDown, role: "separator", children: [_jsx("span", { className: cn('absolute bg-(--ui-stroke-secondary)', horizontal ? 'inset-y-0 left-1/2 w-px -translate-x-1/2' : 'inset-x-0 top-1/2 h-px -translate-y-1/2') }), !disabled && (_jsx("span", { className: cn('absolute bg-(--ui-sash-hover-border) opacity-0 transition-opacity duration-100 group-hover:opacity-100', horizontal
