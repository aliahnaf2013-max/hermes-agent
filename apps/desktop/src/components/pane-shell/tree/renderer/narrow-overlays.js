...
export function NarrowOverlays() {
    ...
    const collapsibles = useMemo(() => panes.filter(p => paneChrome(p).collapsible && inTree.has(p.id) && !hiddenPanes.has(p.id)), [panes, inTree, hiddenPanes]);
    ...
    const sideOf = (c) => (paneChrome(c).placement === 'left' ? 'left' : 'right');
    const revealed = reveal ? collapsibles.find(p => p.id === reveal.id) : undefined;
    const sides = [...new Set(collapsibles.map(sideOf))];
    return (_jsxs(_Fragment, { children: [sides.map(side => (_jsx("div", { className: cn('absolute inset-y-0 z-30 w-1.5', side === 'left' ? 'left-0' : 'right-0'), onMouseEnter: () => {
                    const first = collapsibles.find(p => sideOf(p) === side);
                    if (first) {
                        setReveal(current => (current?.pinned ? current : { id: first.id, pinned: false }));
                    }
                } }, side))), revealed && (_jsx("div", { className: cn('absolute inset-y-0 z-40 flex flex-col overflow-hidden bg-(--ui-sidebar-surface-background) shadow-2xl', sideOf(revealed) === 'left'
                    ? 'left-0 border-r border-(--ui-stroke-secondary)'
                    : 'right-0 border-l border-(--ui-stroke-secondary)'), onMouseLeave: () => setReveal(current => (current?.pinned ? current : null)), 
                // Match the pane's docked width (sessions ~237px, files its rail
                // width) instead of a fat fixed 20rem — capped for tiny screens.
                style: { width: `min(${revealed.data?.width ?? '18rem'}, 85vw)` }, children: _jsx(ContribBoundary, { id: revealed.id, children: revealed.render?.() }) }))] }));
}
