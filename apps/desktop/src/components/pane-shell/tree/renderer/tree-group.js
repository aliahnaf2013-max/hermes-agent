...
import { $dropHint, $hiddenTreePanes, $layoutTree, $narrowViewport, $treeDragging, activateTreePane, closeTreePane, moveTreePane, SESSION_TILE_DRAG, setTreeGroupHeaderHidden, splitTreeZone, toggleTreeGroupMinimized } from '../store';
import { startPaneDrag } from './drag-session';
import { paneChrome } from './track-model';
const DIRECTION_ORDER = ['right', 'bottom', 'left', 'top'];
const DIRECTION_ARROW = { bottom: '↓', left: '←', right: '→', top: '↑' };
...
function ZoneMenu({ children, closable, directions, headerHidden, minimized, nodeId }) {
    const { t } = useI18n();
    return (_jsxs(ContextMenu, { children: [_jsx(ContextMenuTrigger, { asChild: true, children: children }), _jsxs(ContextMenuContent, { children: [directions.map(direction => (_jsx(ContextMenuItem, { onSelect: direction.run, children: direction.label }, direction.side))), _jsx(ContextMenuItem, { onSelect: () => setTreeGroupHeaderHidden(nodeId, !headerHidden), children: headerHidden ? t.zones.showHeader : t.zones.hideHeader }), _jsx(ContextMenuItem, { onSelect: () => toggleTreeGroupMinimized(nodeId, !minimized), children: minimized ? t.zones.restore : t.zones.minimize }), closable?.() !== undefined && (_jsx(ContextMenuItem, { onSelect: () => {
...
export function TreeGroup({ node, parentAxis, railSide = 'left' }) {
    const { t } = useI18n();
    ...
    const menuPaneRef = useRef(undefined);
    ...
    const paneFor = (id) => panes.find(p => p.id === id);
    ...
    const paneShown = (id) => Boolean(paneFor(id)) && (editMode || !hiddenPanes.has(id)) && !(narrow && paneChrome(paneFor(id)).collapsible);
    ...
    const menuDirections = shown.length > 1
    ...
    return (_jsxs("div", { className: "relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-(--ui-bg-editor)", "data-tree-group": node.id, "data-zone-header": headerVisible || undefined, onDoubleClick: onZoneDoubleClick, ref: ref, style: wcOverlap ? { paddingTop: wcOverlap.y + wcOverlap.height } : undefined, children: [wcOverlap && (_jsx("div", { "aria-hidden": "true", className: "pointer-events-none absolute z-10 [-webkit-app-region:drag]", style: { height: wcOverlap.height, left: wcOverlap.x, top: wcOverlap.y, width: wcOverlap.width } })), verticalCollapse && (_jsx(ZoneMenu, { ...zoneMenu, children: _jsx("div", { className: cn('flex h-full w-7 shrink-0 cursor-pointer select-none flex-col items-stretch bg-(--pane-tab-strip-bg) [--pane-tab-strip-bg:var(--theme-card-seed)]', 
                    // Strip line faces the content the zone collapsed away from.
                    railSide === 'right' ? PANE_TAB_STRIP_LINE_LEFT : PANE_TAB_STRIP_LINE_RIGHT), onClick: () => toggleTreeGroupMinimized(node.id, false), title: t.zones.restore, children: _jsx("div", { className: "flex min-h-0 flex-col overflow-y-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden", role: "tablist", children: shown.map(paneId => {
                            ...
                            , { "aria-selected": paneId === activeId, "data-tree-tab": paneId, onClick: event => {
                                    event.stopPropagation();
                                    toggleTreeGroupMinimized(node.id, false);
                                    activateTreePane(node.id, paneId);
                                }, onClose: closeable ? () => closeTreePane(paneId) : undefined, role: "tab", side: railSide, vertical: true, children: _jsx(PaneTabLabel, { children: title }) }, paneId));
                        }) }) }) })), headerVisible && (_jsx(ZoneMenu, { ...zoneMenu, children: _jsxs("div", { 
                    ...
                    className: cn('group/pane-header relative flex h-7 shrink-0 select-none bg-(--pane-tab-strip-bg) [-webkit-app-region:no-drag] [--pane-tab-active-bg:var(--ui-sidebar-surface-background)] [--pane-tab-strip-bg:var(--theme-card-seed)]', PANE_TAB_STRIP_LINE), "data-zone-tabstrip": node.id, onContextMenu: e => {
                        menuPaneRef.current =
                            e.target.closest('[data-tree-tab]')?.getAttribute('data-tree-tab') ?? undefined;
                    }, onPointerDown: e => 
                    ...
                    startPaneDrag(activeId, e, () => toggleTreeGroupMinimized(node.id, !node.minimized), undefined, hideHeaderDoubleTap), ref: stripRef, style: { cursor: 'grab' }, children: [_jsx("div", { className: "flex min-w-0 flex-1 overflow-x-auto overflow-y-hidden [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden", role: "tablist", children: shown.map(paneId => {
                                const isActive = paneId === activeId && !node.minimized;
                                const closeable = !paneChrome(paneFor(paneId)).uncloseable;
                                const title = paneFor(paneId)?.title ?? paneId;
                                return (_jsx(PaneTab, { active: isActive, "aria-selected": isActive, "data-tree-tab": paneId, onClose: closeable ? () => closeTreePane(paneId) : undefined, onPointerDown: e => startPaneDrag(paneId, e, () => {
                                        ...
                                        activateTreePane(node.id, paneId);
                                    }, stripRef.current ? { groupId: node.id, strip: stripRef.current } : undefined, hideHeaderDoubleTap), role: "tab", style: { cursor: 'grab' }, children: _jsx(PaneTabLabel, { children: title }) }, paneId));
                            }) }), _jsx("button", { "aria-label": node.minimized ? t.zones.restore : t.zones.minimize, className: "mx-1 grid size-5 shrink-0 place-items-center self-center rounded-md text-(--ui-text-tertiary) opacity-0 transition-opacity hover:bg-(--ui-control-hover-background) hover:text-foreground focus-visible:opacity-100 group-hover/pane-header:opacity-100", onClick: () => toggleTreeGroupMinimized(node.id, !node.minimized), onPointerDown: e => e.stopPropagation(), type: "button", children: _jsx(Codicon, { name: node.minimized ? 'chevron-down' : 'chevron-up', size: "0.75rem" }) }), _jsx(StripDropCaret, { groupId: node.id, stripRef: stripRef })] }) })), !node.minimized && (_jsx("div", { className: "relative min-h-0 min-w-0 flex-1 overflow-auto", children: isEmpty ? (_jsx("div", { className: "grid h-full place-items-center", children: _jsx(DecodeText, { className: "text-(--ui-text-quaternary)", cursor: true, prefix: 1, text: "HERMES" }) })) : active?.render ? (_jsx(ContribBoundary, { id: active.id, children: active.render() })) : (_jsx("div", { className: "p-3 font-mono text-[11px] text-(--ui-text-quaternary)", children: t.zones.missingPane(activeId) })) })), editMode && !dragging && !isEmpty && !node.minimized && (_jsx(ZoneMenu, { ...zoneMenu, children: _jsx("div", { 
                    ...
                    className: "absolute inset-x-0 bottom-0 z-50 flex cursor-grab items-center justify-center outline-1 -outline-offset-2 outline-dashed backdrop-blur-[2px]", onPointerDown: e => startPaneDrag(activeId, e), style: {
                        top: headerVisible ? 28 : 0,
                        background: 'color-mix(in srgb, var(--ui-accent) 6%, color-mix(in srgb, var(--ui-bg-chrome) 55%, transparent))',
                        outlineColor: 'color-mix(in srgb, var(--ui-accent) 55%, transparent)'
                    }, children: _jsxs("span", { className: "flex max-w-[calc(100%-1rem)] items-center gap-1.5 rounded-md border border-(--ui-stroke-secondary) bg-popover px-2 py-1 text-[0.64rem] font-semibold uppercase tracking-[0.16em] text-(--ui-text-secondary)", children: [_jsx(Codicon, { className: "shrink-0", name: "gripper", size: "0.8125rem" }), _jsx("span", { className: "min-w-0 truncate", children: active?.title ?? activeId })] }) }) })), _jsx(ZoneDropOverlay, { isEmpty: isEmpty, node: node })] }));
}
// ---------------------------------------------------------------------------
 ...
 */
function StripDropCaret({ groupId, stripRef }) {
    ...
    const tabs = [...strip.querySelectorAll('[data-tree-tab]')];
    ...
    return (_jsx("span", { "aria-hidden": true, className: "pointer-events-none absolute z-50 w-px -translate-x-1/2 bg-black dark:bg-white", style: {
 ...
 */
function ZoneDropOverlay({ isEmpty, node }) {
    const { t } = useI18n();
    const dragging = useStore($treeDragging);
    const hint = useStore($dropHint);
    if (dragging === null) {
    ...
    return (_jsx("div", { className: "pointer-events-none absolute inset-0 z-40", style: { animation: `hermes-zone-fade ${OVERLAY_FADE_MS}ms linear both` }, children: _jsx("div", { className: cn(DROP_SHEET_CLASS, 
            ...
            active && !centerLink && DROP_SHEET_BLUR_CLASS, centerLink && 'opacity-0'), style: {
                ...REGION[pos],
            ...
            }, children: pill && _jsx(DropPill, { icon: pill.icon, children: pill.label }) }) }));
}
