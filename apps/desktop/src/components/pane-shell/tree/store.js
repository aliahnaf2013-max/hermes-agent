...
import { allPaneIds, findGroup, findGroupOfPane, groupLeafIds, insertAtGroup, isLayoutNode, mergeZonesWithPane as mergeZonesWithPaneOp, mirrorTreeHorizontal, movePane as movePaneOp, normalize, removePane, reorderPaneInGroup as reorderPaneInGroupOp, setActivePane as setActivePaneOp, setGroupHeaderHidden as setGroupHeaderHiddenOp, setGroupMinimized, setSplitWeights as setSplitWeightsOp, splitGroupZone as splitGroupZoneOp } from './model';
...
export const $layoutTree = atom(loadPersisted());
/**
 ...
 */
export const $activePresetId = atom(readKey('hermes.desktop.layoutPreset.active') ?? 'default');
export function markActivePreset(id) {
...
export const $treeDragging = atom(null);
 ...
 */
export const $hiddenTreePanes = atom(new Set());
...
function toggledSet(set, item, present) {
...
export function setTreePaneHidden(paneId, hidden) {
...
function loadDismissed() {
    return new Set(readJson(DISMISSED_KEY) ?? []);
}
export const $dismissedPanes = atom(loadDismissed());
function saveDismissed(next) {
    $dismissedPanes.set(next);
    writeJson(DISMISSED_KEY, next.size === 0 ? null : [...next]);
}
function setDismissed(paneId, dismissed) {
...
export function registerPaneCloser(paneId, close) {
 ...
 */
export function registerPaneOpener(paneId, open) {
...
export function registerLayoutResetHandler(fn) {
...
export function noteActiveTreeGroup(groupId) {
...
export function trackActiveTreeGroup() {
    const track = (event) => {
        const el = event.target instanceof HTMLElement ? event.target : null;
        const groupId = el?.closest('[data-tree-group]')?.dataset.treeGroup;
        if (groupId) {
 ...
 *  zone. The ⌘W target. */
export function focusedTreePane() {
    const tree = $layoutTree.get();
    if (!tree) {
        return null;
    }
    const el = typeof document !== 'undefined' ? document.activeElement : null;
    const domGroup = el?.closest('[data-tree-group]')?.dataset.treeGroup;
...
export function closeFocusedTreeTab() {
    const active = focusedTreePane();
    if (!active) {
        return false;
    }
    const uncloseable = registry.getArea('panes').find(c => c.id === active)?.data
        ?.uncloseable;
    if (uncloseable) {
        return false;
    }
...
export function removeTreePane(paneId) {
    const tree = $layoutTree.get();
    if (tree) {
        commit(removePane(tree, paneId));
    }
}
export function closeTreePane(paneId) {
...
export const $collapsedTreeSides = atom(new Set());
...
const sideOpeners = {};
export function setTreeSideCollapsed(side, collapsed) {
 ...
 */
export function layoutHasRootSide(side) {
    const tree = $layoutTree.get();
 ...
 */
function restoreDismissedSidePanes(side) {
    ...
    for (const pane of registry.getArea('panes')) {
        ...
        const placement = pane.data?.placement;
...
export function bindTreeSideVisibility(side, $open, setOpen) {
...
export function treeSideOfPane(paneId) {
    const tree = $layoutTree.get();
    ...
    const placementOf = (id) => registry.getArea('panes').find(c => c.id === id)?.data?.placement;
 ...
 */
export function revealTreePane(paneId) {
...
export const $dropHint = atom(null);
/**
 ...
 *  must stand down. */
export const $sessionTileEdgeHover = computed([$treeDragging, $dropHint], (dragging, hint) => dragging === SESSION_TILE_DRAG && ((hint?.pos !== undefined && hint.pos !== 'center') || hint?.stack !== undefined));
/**
 ...
 */
function adoptMissingPanes(target, source) {
 ...
 * panes it's missing.
 */
export function declareDefaultTree(tree) {
    defaultTree = tree;
...
function adoptContributedPanes() {
    const tree = $layoutTree.get();
    if (!tree) {
        return;
    }
    const panes = registry.getArea('panes');
    const dataOf = (paneId) => panes.find(c => c.id === paneId)?.data;
    const placementOf = (paneId) => dataOf(paneId)?.placement;
    ...
    for (const pane of missing) {
        ...
        const anchor = (dock && allPaneIds(next).includes(dock.pane) ? dock.pane : undefined) ??
...
export function watchContributedPanes() {
    adoptContributedPanes();
...
export const $userPlacedPanes = atom(new Set(readJson(USER_PLACED_KEY) ?? []));
function saveUserPlaced(next) {
    $userPlacedPanes.set(next);
    writeJson(USER_PLACED_KEY, next.size === 0 ? null : [...next]);
}
function markPaneUserPlaced(paneId) {
 ...
 * shape no-op.
 */
export function dockPaneBeside(paneId, anchorPaneId) {
    const tree = $layoutTree.get();
    ...
    const anchorRightOfMain = !!mainId && !anchor.panes.includes(mainId) && order.indexOf(anchorPaneId) > order.indexOf(mainId);
    const pos = anchorRightOfMain ? 'left' : 'right';
...
export function moveTreePane(paneId, target) {
    const tree = $layoutTree.get();
    if (!tree) {
        return;
    }
 ...
 */
export function applyTree(tree, presetId) {
    ...
    for (const paneId of allPaneIds(tree)) {
        const data = panes.find(c => c.id === paneId)?.data;
 ...
 */
export function mergeTreeZones(groupIds, paneId, fallbackGroupId) {
    const tree = $layoutTree.get();
    if (!tree) {
        return;
    }
...
export function activateTreePane(groupId, paneId) {
    const tree = $layoutTree.get();
    if (tree) {
        commit(setActivePaneOp(tree, groupId, paneId));
    }
}
export function reorderTreePane(groupId, paneId, toIndex) {
    const tree = $layoutTree.get();
    if (tree) {
...
export function splitTreeZone(groupId, side, movePaneId) {
    const tree = $layoutTree.get();
    if (tree) {
...
export function toggleTreeGroupMinimized(groupId, minimized) {
    const tree = $layoutTree.get();
    if (tree) {
...
export function setTreeGroupHeaderHidden(groupId, headerHidden) {
    const tree = $layoutTree.get();
    if (tree) {
        commit(setGroupHeaderHiddenOp(tree, groupId, headerHidden));
    }
}
export function setTreeSplitWeights(splitId, weights) {
    const tree = $layoutTree.get();
    if (tree) {
...
function findSplitWeights(node, splitId) {
    if (node.type !== 'split') {
        return null;
    }
    if (node.id === splitId) {
 ...
 */
export function presetSplitWeights(splitId, length) {
