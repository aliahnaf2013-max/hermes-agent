...
export function prefixSum(list) {
    const result = [0];
    let sum = 0;
...
function adjacentDifference(list) {
    if (list.length <= 1) {
        return [];
    }
    const result = [];
...
// ---------------------------------------------------------------------------
export function modelToZones(model) {
    ...
    const indexCount = new Array(zoneCount).fill(0);
    const indexRowLow = new Array(zoneCount).fill(Number.MAX_SAFE_INTEGER);
    const indexRowHigh = new Array(zoneCount).fill(0);
    const indexColLow = new Array(zoneCount).fill(Number.MAX_SAFE_INTEGER);
    const indexColHigh = new Array(zoneCount).fill(0);
    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
    ...
    if (model.rowPercents.length !== rows ||
...
export function modelToResizers(model) {
...
// ---------------------------------------------------------------------------
export function zonesToModel(zones) {
    ...
    const model = {
        ...
        cellChildMap: Array.from({ length: yCoords.length - 1 }, () => new Array(xCoords.length - 1).fill(0))
    };
...
// ---------------------------------------------------------------------------
function computeClosure(zones, indices) {
...
export function mergeClosureIndices(model, indices) {
    const zones = modelToZones(model);
    return zones ? computeClosure(zones, indices).indices : [];
}
export function doMerge(model, indices) {
    if (indices.length === 0) {
        return model;
    }
    const zones = modelToZones(model);
...
// ---------------------------------------------------------------------------
export function canSplit(model, zoneIndex, position, orientation) {
    const zones = modelToZones(model);
...
export function splitZone(model, zoneIndex, position, orientation) {
...
// ---------------------------------------------------------------------------
export function canDrag(model, resizerIndex, delta) {
    const zones = modelToZones(model);
...
export function dragResizer(model, resizerIndex, delta) {
...
export function initColumns(count) {
    return {
        rows: 1,
        columns: count,
...
export function initRows(count) {
    return {
        rows: count,
        columns: 1,
...
export function initGrid(zoneCount) {
    let rows = 1;
    ...
    const model = {
        ...
        cellChildMap: Array.from({ length: rows }, () => new Array(cols).fill(0))
    };
    let index = 0;
    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
 ...
 */
export function initPriorityGrid(zoneCount) {
    if (zoneCount === 2) {
...
export function isGridValid(model) {
    ...
    if (!Array.isArray(m.rowPercents) ||
    ...
    if (!Array.isArray(m.cellChildMap) ||
