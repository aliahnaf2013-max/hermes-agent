...
import { deriveProviderShape, isRemoteConfig, isRemoteReauthError, isRemoteReauthFailure, signInLabel } from './boot-failure-reauth';
...
describe('isRemoteReauthFailure', () => {
    ...
    it('true when the indicator reads connected but the boot error is auth-shaped (expired session)', () => {
        expect(isRemoteReauthFailure(config({ remoteOauthConnected: true }), 'Your remote gateway session has expired.')).toBe(true);
    });
...
describe('deriveProviderShape', () => {
    ...
    it('password shape when the sole provider supports password', () => {
        expect(deriveProviderShape([{ name: 'basic', displayName: 'Username & Password', supportsPassword: true }])).toEqual({ isPassword: true, providerLabel: 'Username & Password' });
    });
    ...
    it('falls back to name when displayName is empty', () => {
        expect(deriveProviderShape([{ name: 'basic', displayName: '', supportsPassword: true }]).providerLabel).toBe('basic');
    });
});
describe('signInLabel', () => {
    it('password gateway gets the plain "Sign in to remote gateway" copy', () => {
        expect(signInLabel({ url: 'x', isPassword: true, providerLabel: 'Username & Password' })).toBe('Sign in to remote gateway');
    });
    it('OAuth gateway names the provider', () => {
        expect(signInLabel({ url: 'x', isPassword: false, providerLabel: 'Nous Research' })).toBe('Sign in with Nous Research');
    });
