...
function readClarifyArgs(args) {
    if (!args || typeof args !== 'object') {
        return {};
    }
    const row = args;
    const choices = Array.isArray(row.choices) ? row.choices.filter((c) => typeof c === 'string') : null;
    return {
...
const OPTION_ROW_CLASS = 'flex w-full items-start gap-2 rounded-[0.25rem] px-1.5 py-1 text-left disabled:cursor-not-allowed disabled:opacity-50';
...
const FREEFORM_INPUT_CLASS = 'field-sizing-content max-h-40 min-h-0 w-full resize-none bg-transparent p-0 leading-(--conversation-line-height) text-(--ui-text-primary) outline-none placeholder:text-(--ui-text-tertiary) disabled:opacity-50';
...
const CLARIFY_SHELL_CLASS = 'my-1.5 rounded-md border border-primary/20 bg-(--ui-chat-surface-background) text-[length:var(--conversation-text-font-size)] text-(--ui-text-primary)';
function ClarifyShell({ children, className, ...props }) {
    return (_jsx("div", { className: cn(CLARIFY_SHELL_CLASS, className), "data-slot": "clarify-inline", ...props, children: children }));
}
...
function KeyBadge({ char, preview, selected }) {
    return (_jsx(Kbd, { className: cn('mt-px', selected && 'border-primary bg-primary text-white shadow-none', !selected && preview && 'border-primary text-primary shadow-none'), size: "sm", children: char }));
}
export const ClarifyTool = (props) => {
    ...
    if (!isPending) {
        return _jsx(ToolFallback, { ...props });
    }
    return _jsx(ClarifyToolPending, { ...props });
};
function ClarifyToolPending({ args }) {
    const { t } = useI18n();
    ...
    const choices = useMemo(() => fromArgs.choices ?? matchingRequest?.choices ?? [], [fromArgs.choices, matchingRequest?.choices]);
    ...
    const handleTextareaKey = useCallback((event) => {
    ...
    if (loading) {
        return (_jsx(ClarifyShell, { "aria-label": copy.loadingQuestion, className: "grid min-h-12 place-items-center px-2.5 py-3", role: "status", children: _jsx(Loader2, { "aria-hidden": true, className: "size-4 animate-spin text-(--ui-text-tertiary)" }) }));
    }
    ...
    return (_jsxs(ClarifyShell, { className: "grid gap-2 px-2.5 py-2", children: [_jsxs("div", { className: "flex items-start gap-2", children: [_jsx("span", { className: "flex-1 whitespace-pre-wrap font-medium leading-(--conversation-line-height)", children: question }), _jsx(MessageQuestion, { "aria-hidden": true, className: "mt-px size-4 shrink-0 text-(--ui-text-tertiary)" })] }), _jsxs("form", { className: "grid gap-2", onSubmit: handleSubmit, children: [hasChoices ? (_jsxs("div", { className: "grid gap-px", role: "group", children: [choices.map((choice, index) => (_jsxs("button", { className: cn(OPTION_ROW_CLASS, 'text-(--ui-text-secondary) hover:bg-(--chrome-action-hover) hover:text-(--ui-text-primary)', selectedChoice === choice && 'text-(--ui-text-primary)'), "data-choice": true, disabled: submitting, onClick: () => selectChoice(choice), type: "button", children: [_jsx(KeyBadge, { char: letterFor(index), selected: selectedChoice === choice }), _jsx("span", { className: "flex-1 wrap-anywhere", children: choice })] }, `${index}-${choice}`))), _jsxs("label", { className: cn(OPTION_ROW_CLASS, 'focus-within:bg-(--chrome-action-hover)'), children: [_jsx(KeyBadge, { char: letterFor(choices.length), preview: otherFocused, selected: Boolean(trimmedDraft) }), _jsx("textarea", { className: FREEFORM_INPUT_CLASS, disabled: submitting, onBlur: () => setOtherFocused(false), onChange: event => onDraftChange(event.target.value), 
                                        ...
                                        }, onKeyDown: handleTextareaKey, placeholder: copy.other, ref: textareaRef, rows: 1, value: draft })] })] })) : (_jsx(Textarea, { className: FREEFORM_INPUT_CLASS, disabled: submitting, onChange: event => onDraftChange(event.target.value), onKeyDown: handleTextareaKey, placeholder: copy.placeholder, ref: textareaRef, rows: 1, value: draft })), _jsxs("div", { className: "flex items-center justify-end gap-1", children: [_jsx(Button, { disabled: submitting, onClick: () => void respond(''), size: "xs", type: "button", variant: "text", children: copy.skip }), _jsx(Button, { disabled: submitting || !pendingAnswer, size: "xs", type: "submit", children: submitting ? (_jsx(Loader2, { className: "size-3 animate-spin" })) : (_jsxs(_Fragment, { children: [copy.continueLabel, _jsx("span", { "aria-hidden": true, className: "ml-0.5 text-[0.625rem] opacity-70", children: "\u23CE" })] })) })] })] })] }));
}
