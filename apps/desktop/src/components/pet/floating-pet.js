...
import { $petAtRest, $petInfo, $petRoam, $petRoamDir, clearPetUnread, petProfile, setPetInfo } from '@/store/pet';
...
function samePetRevision(info, meta) {
    return (info.enabled &&
...
export function FloatingPet() {
    ...
    const clamp = useCallback(({ x, y }) => clampPoint(x, y, petW, petH), [petW, petH]);
    ...
    useEffect(() => {
        ...
        const pull = async () => {
            try {
                ...
                const next = await requestGateway('pet.info', { profile: petProfile() });
                if (!cancelled && next) {
    ...
    const onScale = useCallback((next, { clientX, clientY, ratio }) => {
        setPetScale(requestGateway, next);
        setPosition(prev => {
            const at = clampPoint(clientX - (clientX - prev.x) * ratio, clientY - (clientY - prev.y) * ratio, (info.frameW ?? 192) * next, (info.frameH ?? 208) * next);
    ...
    const commitRoamPosition = useCallback((point) => {
        setPosition(point);
    ...
    return (_jsxs("div", { onPointerDown: onPointerDown, onPointerMove: onPointerMove, onPointerUp: onPointerUp, ref: containerRef, style: {
            cursor: 'grab',
            left: position.x,
            pointerEvents: 'auto',
            position: 'fixed',
            ...
            zIndex: 60
                    ...
                    width: shadowW,
                    zIndex: 0
                } }), _jsx("div", { ref: spriteWrapRef, style: {
                    lineHeight: 0,
                    position: 'relative',
                    transform: roamDir !== 0 ? (walk.mirror ? 'scaleX(-1)' : 'none') : facing(position.x, petW),
                    zIndex: 1
                }, children: _jsx(PetSprite, { info: info, rowOverride: walk.row }) }), _jsx(PetHeartField, { petH: petH, petW: petW })] }));
}
