...
export function GatewayConnectingOverlay() {
    ...
    const connecting = !coldBootDoneRef.current &&
        !gatewaySwitching &&
        gatewayState !== 'open' &&
        !boot.error &&
        initialBootActive;
    ...
    return (_jsx("div", { className: cn('fixed inset-0 z-[1200] grid place-items-center bg-(--ui-chat-surface-background) transition-opacity duration-500 ease-out', overlayHidden ? 'pointer-events-none opacity-0' : 'opacity-100'), children: _jsx(DecodeText, { active: phase === 'live' && (previewing || connecting), className: cn('pl-[0.4em] text-(--theme-primary) transition duration-300 ease-out', leaving ? 'translate-y-2 opacity-0 saturate-0' : 'translate-y-0 opacity-100 saturate-100'), cursor: true, prefix: 4, text: TEXT }) }));
}
