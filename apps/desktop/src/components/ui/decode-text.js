...
const TICK_MS = 45;
const HOLD_TICKS = 16;
function scrambled(tail, resolvedCount) {
    return Array.from(tail, (ch, i) => ch === ' ' || i < resolvedCount ? ch : DECODE_SCRAMBLE_CHARS[(Math.random() * DECODE_SCRAMBLE_CHARS.length) | 0]).join('');
}
export function DecodeText({ active = true, className, cursor = false, loop = true, prefix = 0, text, ...props }) {
    ...
    return (_jsxs("span", { className: cn('inline-flex items-center font-mono text-[0.64rem] font-semibold uppercase tracking-[0.4em] tabular-nums', className), ...props, children: [cursor && _jsx("style", { children: '@keyframes decode-cursor { 0%, 49% { opacity: 1 } 50%, 100% { opacity: 0 } }' }), staticPrefix, tail, cursor && (_jsx("span", { "aria-hidden": "true", className: "dither ml-0.5 inline-block size-2 shrink-0 -translate-y-px rounded-[1px]", style: { animation: 'decode-cursor 1s step-end infinite' } }))] }));
}
