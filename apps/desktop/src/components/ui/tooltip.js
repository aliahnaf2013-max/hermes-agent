...
function TooltipContent({ className, sideOffset = 6, children, ...props }) {
    return (_jsx(TooltipPrimitive.Portal, { children: _jsx(TooltipPrimitive.Content
        ...
        , { 
            ...
            // pointer-events-none: the tip must never steal hover/clicks from the
            // chrome underneath (titlebar tools, adjacent tabs, etc.).
            className: cn('pointer-events-none z-[200] w-fit max-w-64 select-none', className), "data-slot": "tooltip-content", sideOffset: sideOffset, ...props, children: _jsx("span", { className: "box-decoration-clone inline bg-foreground px-1.5 py-1 text-[11px] font-bold leading-normal text-background [font-family:Arial,sans-serif] [&>*]:!inline-flex", children: children }) }) }));
}
...
function TipHintLabel({ text, hint }) {
    if (!hint) {
        return _jsx(_Fragment, { children: text });
    }
    return (_jsxs("span", { className: "inline-flex items-center gap-2", children: [_jsx("span", { children: text }), _jsx("span", { className: "opacity-55", children: hint })] }));
}
