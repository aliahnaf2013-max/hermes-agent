...
export function DropPill({ children, className, icon }) {
    return (_jsxs("div", { className: cn('flex items-center gap-2 rounded-full border border-[color-mix(in_srgb,var(--dt-composer-ring)_45%,transparent)] bg-[color-mix(in_srgb,var(--dt-card)_92%,transparent)] px-4 py-2 text-[0.8125rem] font-medium text-foreground shadow-composer', className), children: [_jsx(Codicon, { className: "shrink-0 text-(--ui-accent)", name: icon, size: "1rem" }), children] }));
}
