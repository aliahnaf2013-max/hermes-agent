...
export const KEYBIND_CATEGORIES = ['composer', 'profiles', 'session', 'navigation', 'view'];
...
function comboForSlot(slot) {
    return slot <= 9 ? `mod+${slot}` : `mod+alt+${slot - 9}`;
}
const PROFILE_SWITCH_ACTIONS = Array.from({ length: PROFILE_SLOT_COUNT }, (_, i) => ({
...
const SESSION_SLOT_ACTIONS = Array.from({ length: SESSION_SLOT_COUNT }, (_, i) => ({
    id: `session.slot.${i + 1}`,
    category: 'session',
    defaults: [`ctrl+${i + 1}`]
}));
...
export const KEYBIND_ACTION_IDS = KEYBIND_ACTIONS.map(action => action.id);
...
export function allKeybindActions() {
    return [
        ...KEYBIND_ACTIONS,
        ...contributedKeybinds().map(k => ({
            id: k.id,
            category: k.category ?? 'view',
            defaults: k.defaults ?? [],
            label: k.label
        }))
    ];
...
export function contributedKeybindHandler(id) {
