...
import { dropPlugin, pluginActive, publishPlugin } from './plugins-store';
...
function rewriteSpecifiers(source) {
    ...
    return source.replace(importSpecifierRe(), (whole, pre, quote, spec) => map[spec] ? `${pre}${quote}${map[spec]}${quote}` : whole);
}
...
export async function loadRuntimePlugin(source, origin, options = {}) {
    ...
    try {
        ...
        if (unsupported.length > 0) {
            throw new Error(`unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
...
let watching = false;
let scanning = false;
async function loadDiskPlugin(name, file) {
...
async function scanDiskPlugins() {
    const desktop = window.hermesDesktop;
    ...
    try {
        ...
        for (const dir of entries.filter(e => e.isDirectory)) {
            ...
            const record = { file, id: null, watchId: null };
...
export function watchRuntimePlugins() {
    const desktop = window.hermesDesktop;
    if (watching || !desktop) {
        return;
    }
